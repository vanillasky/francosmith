...
{
	static $part_idx = -1; // 2013-09-08 slowj 디자인관리 part idx
	function _compile_template($tpl, $tpl_path, $cpl_base, $cpl_head, $fid = '') // 2013-09-08 slowj 디자인관리 fid 인자추가
	{
		...
		if (!$this->skin) {
			$tmp_cdir = explode('/', $this->compile_dir);
			$this->skin = $tmp_cdir[count($tmp_cdir) - 1];
			unset($tmp_cdir);
		}
		// 2013-09-04 slowj 디자인 미리보기 수정
	...
	// make compile directory
		...
		// 2013-09-04 slowj 디자인 미리보기 수정
		if ($this->gd_preview) $cpl_path = _template_preview_path($cpl_path);
		...
		if (!@is_file($cpl_path)) {
			...
			for ($nodir=0,$i=1,$s=count($dir)-1; $i<$s; $i++) {
				...
				if ($nodir or @!is_dir($path) and $nodir=1) {
					if ( !in_array($dir[0], array('.','..')) && (stripos($path, $_SERVER['DOCUMENT_ROOT']) === false || is_dir($path)) ) continue;
	...
	// get template
		// 2013-09-04 slowj 디자인 미리보기
		$tmp_tpl_path = $tpl_path;
		$after_tpl_path = strtolower(str_replace(str_replace('\\', '/', realpath($this->template_dir)), '', str_replace('\\', '/', realpath($tpl_path))));

		// 미리보기 파일인 경우 경로변경, 아니거나 미리보기 파일이 없으면 기존 경로의 소스 가져오기
		if ($this->gd_preview && $after_tpl_path == $tpl->gd_preview_file) {
			$preview_tpl_path = _template_preview_path($tpl_path, 'tpl');
			if (file_exists($preview_tpl_path)) $tpl_path = $preview_tpl_path;
			unset($preview_tpl_path);
		}
		...
		if ($source_size = filesize($tpl_path)) {
			...
			if ($this->gd_srcview) {
				if (!function_exists('remove_nl')) {
					function remove_nl($matches) {
						return str_replace('ｎｌ', 'ＲＬ', $matches[0]);
					}
				}

				if ($this->gd_preview && $after_tpl_path == $tpl->gd_preview_file) {
					$tmp_dir = explode('/', str_replace('\\', '/', realpath($this->template_dir)));
					$skin_dir = implode('/', array_splice($tmp_dir, -2));
					$tmp_dir = implode('/', $tmp_dir);

					$str_real_tpl_path = str_replace('\\', '/', str_replace($tmp_dir, '/../../', str_replace('\\', '/', realpath($tpl_path))));
				}
				else {
					$str_real_tpl_path = str_replace('\\', '/', str_replace(realpath($this->template_dir), '', realpath($tpl_path)));
				}
				$str_tpl_path = str_replace('\\', '/', str_replace(realpath($this->template_dir), '', realpath($tmp_tpl_path)));

				// script/style 내부의 line 주석은 제거.(nl -> NL로 변경)
				$pattern = array('/<\s*(script|style)[^>]*>(.*?)<\s*\/(script|style)[^>]*>/', '/<\?(.*?)\?>/', '/<!--(.*?)-->/');
				$source=preg_replace_callback($pattern, 'remove_nl', str_replace(chr(10), 'ｎｌ', $source));

				// fid 변형.
				$fid = Template_::$main_tpl.' '.htmlspecialchars($fid).'_'.(++self::$part_idx);

				// nl은 줄바꿈nl로, NL은 그냥 줄바꿈으로 변경.
				$split_source = explode(chr(10), str_replace(array('ｎｌ', 'ＲＬ'), array(chr(10).'ｎｌ', chr(10)), $source));
				$split_source[0] = '<!-- gdline 1"'.$str_tpl_path.'|'.$str_real_tpl_path.'|'.$fid.'" -->' . $split_source[0];
				for($i = 1; $i < count($split_source); ++$i) {
					$split_source[$i] = preg_replace('/^ｎｌ/', '<!-- gdline '.($i+1).'"'.$str_tpl_path.'|'.$str_real_tpl_path.'|'.$fid.'" -->', $split_source[$i]);
				}
				$source = '<!-- gdpart mode="open" fid="'.$fid.'" -->'.implode(chr(10), $split_source).'<!-- gdpart mode="close" fid="'.$fid.'" -->';
			}
			fclose($fp_tpl);
		}
	...
	function _compile_statement($statement)
	{
		...
		case '@' :
			...
			if ($xpr) {
				...
				return array(2, 'if((is_array($TPL_R'.$d.'='.$xpr.')&&!empty($TPL_R'.$d.')) || (is_object($TPL_R'.$d.') && in_array("Countable", class_implements($TPL_R'.$d.')) && $TPL_R'.$d.'->count() > 0)) {');
			}
	...
	function _get_loop_size($depth, $div='')
	{
		...
		foreach ($array as $loop_id => $val) {
			...
			$size .= $this->nl.'if (is_array('.$loop_array.')) $TPL_'.$loop_id.'_'.$depth.'=count('.$loop_array.'); ';
			$size .= 'else if (is_object('.$loop_array.') && in_array("Countable", class_implements('.$loop_array.'))) $TPL_'.$loop_id.'_'.$depth.'='.$loop_array.'->count();';
			$size .= 'else $TPL_'.$loop_id.'_'.$depth.'=0;';
		}
		return $size;
	}
