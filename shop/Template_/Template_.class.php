...
class Template_
{
	...
	var $_include_tpl_max_count = 1000;
	var $_include_tpl_count = array();

	// 2013-09-04 slowj 디자인 미리보기 수정
	static $main_tpl = null;
	var $gd_preview = false;
	var $gd_srcview = false;
	var $gd_preview_file = null;
	function Template_() {
		if ($GLOBALS['ici_admin'] && $_GET['gd_preview'] == '1') {
			if ((preg_match('/^'.str_replace('/', '', $cfg['rootDir']).'/', $_SERVER['PHP_SELF']) && ($GLOBALS['cfg']['tplSkin'] == $GLOBALS['cfg']['tplSkinWork'] || !$GLOBALS['cfg']['tplSkinWork'])) || (preg_match('/^'.str_replace('/', '', $cfg['mobileShopRootDir']).'/', $_SERVER['PHP_SELF']) && ($GLOBALS['cfgMobileShop']['tplSkinMobile'] == $GLOBALS['cfg']['tplSkinMobileWork'] || !$GLOBALS['cfg']['tplSkinMobileWork']))) {
				$this->compile_check = 'dev';
				$this->gd_preview = true;
				// 미리보기 파일.
				if ($_GET['gd_preview_file']) $this->gd_preview_file = preg_replace('/\/+/', '/', '/'.$_GET['gd_preview_file']);
				if ($_GET['gd_srcview'] == '1') $this->gd_srcview = true;
			}
		}
	}
	// 2013-09-04 slowj 디자인 미리보기 수정
	...
	{
		// 2013-09-11 slowj 디자인관리
		if ($this->skin) $skin_nm = $this->skin;
		else {
			$tmp_cdir = explode('/', $this->template_dir);
			$skin_nm = $tmp_cdir[count($tmp_cdir) - 1];
			unset($tmp_cdir);
		}
		// 2013-09-11 slowj 디자인관리
		...
		$tpl=&$this->tpl_[$fid];
		// 최상위 페이지명.
		if (self::$main_tpl == null) self::$main_tpl = $tpl[1];
		$ori_tpl = $tpl[1];
		if ($tpl[0]==='txt') {
			// 2013-09-05 slowj 디자인관리
			ob_start();
			$this->_print_txt($this->_get_compile_path($tpl[1]), $fid);
			$text=ob_get_contents();
			ob_end_clean();
			if ($this->gd_srcview && self::$main_tpl == $ori_tpl) $text=_template_contents_parse($text);
			echo $text;
			if ($this->gd_preview && self::$main_tpl == $ori_tpl) echo '<script type="text/javascript" src="/shop/lib/js/designhelper.js"></script><script>var __gd_dh_skin_nm__ = "'.$skin_nm.'";</script>';
			// 2013-09-05 slowj 디자인관리
			return;
		}
		if ($this->caching) {
...
//				ob_start();
		...
		ob_start();
		if ($tpl[0]==='tpl') {
			$cpl_path=$this->_get_compile_path($tpl[1], $fid);
	...
	{
		if (++$this->_include_tpl_count[$TPL_TPL][$TPL_CPL] > $this->_include_tpl_max_count) {
			$tpl_path = $this->template_dir.'/'.($this->skin?$this->skin.'/':'').$this->tpl_[$TPL_TPL][1];
			$this->exit_('', 'infinite loop founded in <b>'.$tpl_path.'</b>');
		}
	...
	function _get_compile_path($rpath, $fid = '') // 2013-09-08 slowj 디자인관리 fid 인자 추가
	{
		...
		$cpl_base=str_replace('../../skin_today/', '../skin_today/', $cpl_base);
		...
		$compiler->_compile_template($this, $tpl_path, $cpl_base, $cpl_head, $fid);
		return $cpl_path;
	}
...
// 2013-09-04 slowj 디자인 미리보기 수정
function _template_preview_path($path, $path_type='cpl') {
	if ($path_type == 'cpl') {
		$path = str_replace('\\', '/', $path);
		$path = explode('/', $path);
		$path[] = $path[count($path) - 1];
		$path[count($path) - 2] = '__gd__preview';
		$path = implode('/', $path);
	}
	else {
		$tmp_path = explode('/', str_replace('\\', '/', $GLOBALS['tpl']->template_dir));
		$skin_path = array_splice($tmp_path, - 2);
		$tmp_path[] = '_skin_preview';
		$tmp_path = array_merge($tmp_path, $skin_path);
		$tmp_path = implode('/', $tmp_path);

		$file_path = str_replace(realpath($GLOBALS['tpl']->template_dir), '', $path);
		$path = str_replace('\\', '/', realpath($tmp_path .'/'.$file_path));
	}
	return $path;
}

function _template_contents_parse($content) {
	// <!DOCTYPE 이전 주석 제거.
	$content = str_replace(chr(10), 'ｎｌ', $content);
	if (preg_match('/^(.*)\<\!DOCTYPE/i', $content, $matches)) $content = str_replace($matches[1], preg_replace('/<!--\s*(gdline|gdpart)\s*(.*?)\s*-->/', '', $matches[1]), $content);
	$content = str_replace('ｎｌ', chr(10), $content);
