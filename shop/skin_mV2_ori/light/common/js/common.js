...
/*** 홈화면 추가 버튼 ***/
function addHomeButtonForDevice(title){ //즐겨찾기
	...
	var imglength = sm_HomeButtonIconUri.length;
	if(sm_HomeButtonIconUri.charAt(imglength-1)=='/'){
		var imgname = "mobileShopIcon.jpg"
		var sm_HomeButtonIconUri = sm_HomeButtonIconUri.concat(imgname);
	}
	var sm_naver_customUrlScheme= "intent://addshortcut?url="+encodeURI(sm_HomePageUri)+"&icon="+sm_HomeButtonIconUri+"&title="+sm_HomeButtonTitle+"&oq="+sm_HomeButtonTitle+"&serviceCode=nstore&version=7#Intent;scheme=naversearchapp;action=android.intent.action.VIEW;category=android.intent.category.BROWSABLE;package=com.nhn.android.search;end";
	...
	var sm_BlockChrome = sm_UserAgent.indexOf("chrome");
	...
	if(sm_BlockDevice == -2){
		...
		alert("아이폰, 아이패드 계열은 직접 홈버튼추가를 사용하셔야 합니다.");
	}
}
