...
try {
	...
	if (!$cfg_step[2]['sort_type'] || $cfg_step[2]['sort_type'] == '1') {
		...
		//현재 사용중인 테이블
		$mainAutoSort_useTable = $mainAutoSort->getUseTable($cfg_step[2]['sort_type']);
		//최대 상품수
		$mainAutoSort_sortLimit = $mainAutoSort->getSortLimit();
		//해시태그 자동진열
		if((string)$cfg_step[2]['sort_type'] === '5'){
			$sortNum = $mainAutoSort_useTable.".auto_goodsno DESC";
		}
		else {
			$sortNum = "sort".$cfg_step[2]['sort_type']."_".$cfg_step[2]['select_date'];
		}
		$orderby = 'order by '.$sortNum;

		list($add_table, $add_where, $add_group) = $mainAutoSort->getSortTerms($cfg_step[2], $sortNum);
