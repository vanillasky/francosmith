				...
				if(result.success){
					if(!result.listingCnt || listingEnd){
						...
						$.each(result.list, function(i, item){
							...
							if(item.coupon_discount){	//쿠폰할인
								couponImage = '<span class="gl_coupon"></span>';
							}

							if (item.special_discount) {	//상품할인
								specialDiscount = '('+item.special_discount+' 할인)';
							}

							_data.append('<li>');
							_data.append('<dl>');
							_data.append('<dt class="hidden">상품이미지</dt>');
							_data.append('<dd class="gl_img">' + couponImage + item.img_html + '</dd>');
							...
							if (item.strprice) {
								...
								_data.append('<dd class="gl_price">' + item.strprice + '</dd>');
							}
							else if (item.price) {
								_data.append('<dt class="gl_price_title blt">상품가격 : </dt>');
								_data.append('<dd class="gl_price">' + item.price + ' ' + specialDiscount + '</dd>');
