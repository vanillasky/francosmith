...
// 택배사명 조회시, 이나무에서 수정한 경우 키가 맞지 않아 오류가 발생할 수 있으므로, db 의 deliveryno 값을 이용하도록 수정.
$compcode = array();
/*/
...
$compcode['CJ GLS(HTH통합)'] = 'CJ'; // CJ GLS
...
$compcode[4]		= 'KE'; // 대한통운
$compcode[5]		= 'LG'; // 로젠택배
$compcode[7]		= 'AJ'; // 아주택배 = 동원로엑스
$compcode[8]		= 'YC'; // 엘로우캡
$compcode[9]		= 'PO'; // 우체국택배
$compcode[100]		= 'PO'; // 우체국택배(연동)
$compcode[10]		= 'EZ'; // 이젠택배
$compcode[11]		= 'TN'; // 트라넷
$compcode[12]		= 'HJ'; // 한진택배
$compcode[13]		= 'HD'; // 현대택배
$compcode[14]		= 'FE'; // 훼미리택배
$compcode[1]		= 'KB'; // KGB택배
$compcode[2]		= 'KT'; // KT로지스택배
$compcode[21]		= 'FE'; // 동부익스프레스
$compcode[22]		= 'IY'; // 일양로지스
$compcode[32]		= 'IN'; // 이노지스택배
$compcode[20]		= 'HN'; // 하나로택배
$compcode[33]		= 'DS'; // 대신택배
$compcode[18]		= 'RP'; // 우편등기
$compcode[17]		= 'SC'; // SC로지스택배
...
$dlvtype		= empty($compcode[$data['deliveryno']]) ? '01' : '03';			// 등록내용구분(03:발송, 01:수령 중 택일)
...
if ( "03" == $dlvtype )
{
	...
	$dlvcompcode	= $compcode[ $data['deliveryno'] ];				// 배송회사코드
	...
	if ( $dlvcompcode == '' ){											// 수령정보(위 15개사외 택배사 사용시 또는 상점 직접 배송)
		...
		if (empty($rcvdate)) {
			$rcvdate = date_form( $data['ddt'] );						 // 발송일자
		}
