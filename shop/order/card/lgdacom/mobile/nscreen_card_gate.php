...
if( $_POST['settlekind'] == 'o' || $_POST['settlekind'] == 'v' ) {
	$payReqMap['LGD_CASHRECEIPTYN']		= $pg['receipt'];								// 현금영수증 미사용여부(Y:미사용,N:사용)
}
...
if( $_POST['settlekind'] == 'v'){
	...
	$payReqMap['LGD_CASNOTEURL']		= $lguplusReturnUrl.'/cas_noteurl.php';													// 가상계좌 NOTEURL
}

// LGD_RETURNURL 을 설정하여 주시기 바랍니다. 반드시 현재 페이지와 동일한 프로트콜 및  호스트이어야 합니다. 아래 부분을 반드시 수정하십시요.
$payReqMap['LGD_RETURNURL']				= $lguplusReturnUrl.'/mobile/nscreen_card_return.php?page_type='.$page_type;			// 응답수신페이지
...
$payReqMap['LGD_KVPMISPNOTEURL']       	= $lguplusReturnUrl.'/mobile/nscreen_card_return.php?isAsync=Y&page_type='.$page_type;			// 비동기 ISP(ex. 안드로이드) 승인결과를 받는 URL
$payReqMap['LGD_KVPMISPWAPURL']			= $lguplusReturnUrl.'/mobile/nscreen_mispwapurl.php?LGD_OID='.$payReqMap['LGD_OID'];			// 비동기 ISP(ex. 안드로이드) 승인완료후 사용자에게 보여지는 승인완료 URL - ISP 카드 결제시, URL 대신 앱명 입력시, 앱호출함
$payReqMap['LGD_KVPMISPCANCELURL']     	= $lguplusReturnUrl.'/mobile/nscreen_Cancel.php?isAsync=Y&page_type='.$page_type;				// ISP 앱에서 취소시 사용자에게 보여지는 취소 URL
