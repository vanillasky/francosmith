<?php
	...
	if($_GET['isAsync']=='Y'){
		$orderUrl='HTTP://'.$_SERVER['SERVER_NAME'].$cfgMobileShop['mobileShopRootDir']."/ord/order.php";
	...
	$LGD_TID					= $data['tid'];									//LG데이콤으로 부터 내려받은 거래번호(LGD_TID)
 	...
 	$configPath					=$_SERVER['DOCUMENT_ROOT'].$cfg['rootDir']."/conf/lgdacom_mobile";				//LG데이콤에서 제공한 환경파일("/conf/lgdacom.conf") 위치 지정.
