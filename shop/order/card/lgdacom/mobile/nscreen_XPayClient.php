...
class XPayClient
{
	...
	function XPayClient($home_dir, $mode="real")
	{
		...
		else if (!file_exists($home_dir."/mall.conf")) {
			$this->response_code = LGD_ERR_NO_MALL_CONFIG;
			$this->response_msg = "config file [".$home_dir."/mall.conf] does not exist";
			trigger_error($this->response_msg, E_USER_ERROR);
		}
		else if (!file_exists($home_dir."/lgdacom.conf")) {
			$this->response_code = LGD_ERR_NO_LGDACOM_CONFIG;
			$this->response_msg = "config file [".$home_dir."/lgdacom.conf] does not exist";
			trigger_error($this->response_msg, E_USER_ERROR);
		}

		$array1 = parse_ini_file($home_dir . "/mall.conf");
		$array2 = parse_ini_file($home_dir . "/lgdacom.conf");
		$this->config = $array1 + $array2;

		$log_dir	 = str_replace("/conf/lgdacom_mobile","",$home_dir);
		$log_dir	 = str_replace("/conf/lgdacom","",$home_dir);

		$this->log_file = $log_dir.$this->config["log_dir"] . "/log_" . date("Ymd") . ".log";
		// make log directory if does not exist
		if (!file_exists($log_dir.$this->config["log_dir"])) {
			mkdir($log_dir.$this->config["log_dir"], "0777", true);
		}
	...
	function init()
	{
		...
		curl_setopt($this->ch, CURLOPT_CAINFO, $this->home_dir."/ca-bundle.crt");
...
//		$this->log("session id = ".session_id().$_SESSION['tx_counter'], LGD_LOG_FATAL);
	...
	function Patch_TX($filename)
	{
		...
		$tmp_file = tempnam($this->home_dir, "tmp");
		$fp = fopen($tmp_file, "w");
		...
		if ($result == false) {
			...
			if (strncasecmp($type, "text/plain", 10) == 0) {
				...
				copy($tmp_file, $this->home_dir."/".$filename);
				unlink($tmp_file);
				return true;
			}
			else {
