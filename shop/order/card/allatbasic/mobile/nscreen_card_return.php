...
if (forge_order_check($ordno,$_POST['allat_amt']) === false) {
	msg('주문 정보와 결제 정보가 맞질 않습니다. 다시 결제 바랍니다.',$order_fail_page.'?ordno='.$_POST['allat_order_no'],'parent');
	exit();
}
...
if ($naverNcash->useyn == 'Y') {
	...
	if ($ncashResult === false) {
		msg('네이버 마일리지 사용에 실패하였습니다.',$order_fail_page.'?ordno='.$ordno,'parent');
		exit();
	}
}
