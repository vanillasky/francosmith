...
if( !strcmp($REPLYCD,'0000') )
{
	...
	if (empty($crno) === true)
	{
		...
		# 현금영수증신청내역 수정
		$db->query("update gd_cashreceipt set pg='allatbasic',cashreceipt='{$CASH_BILL_NO}',receiptnumber='{$APPROVAL_NO}',tid='{$CASH_BILL_NO}',moddt=now(),status='ACK',errmsg='',receiptlog=concat(if(receiptlog is null,'',receiptlog),'\n{$settlelog}') where crno='{$crno}'");
	...
	if (empty($crno) === true)
	{
		...
		# 현금영수증신청내역 수정
		$db->query("update gd_cashreceipt set pg='allatbasic',errmsg='{$REPLYCD}:{$REPLYMSG}',moddt=now(),receiptlog=concat(if(receiptlog is null,'',receiptlog),'\n{$settlelog}') where crno='{$crno}'");
	}
