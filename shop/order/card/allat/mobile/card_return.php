...
include "../../../../conf/pg_mobile.allat.php";
...
if (forge_order_check($_POST['allat_order_no'],$_POST['allat_amt']) === false) {
	msg('주문 정보와 결제 정보가 맞질 않습니다. 다시 결제 바랍니다.',$cfgMobileShop['mobileShopRootDir']."/ord/order_fail.php?ordno=".$_POST['allat_order_no'],'parent');
	exit();
}
...
if ($naverNcash->useyn == 'Y') {
	if ($_POST['allat_vbank_yn'] == 'Y' || $_POST['allat_vbankes_yn'] == 'Y') $ncashResult = $naverNcash->payment_approval($ordno, false);
	else $ncashResult = $naverNcash->payment_approval($ordno, true);
	if ($ncashResult === false) {
		msg('네이버 마일리지 사용에 실패하였습니다.',$cfgMobileShop['mobileShopRootDir'].'/ord/order_fail.php?ordno='.$ordno,'parent');
		exit();
	}
}
