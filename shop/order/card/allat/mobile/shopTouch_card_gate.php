<?php
	...
	include dirname(__FILE__)."/../../../../conf/pg_mobile.allat.php";
...
<script type="text/javascript">
	...
	function approval_submit(result_cd,result_msg,enc_data) {
		...
		if( result_cd != '0000' ){
			...
			sendFm.action = "http://<?=$_SERVER['HTTP_HOST']?><?=$cfg['rootDir']?>/order/card/allat/mobile/shopTouch_card_return.php";
			sendFm.method = "post";
			sendFm.target = "_self";
			sendFm.submit();
		}
...
<input type="hidden" name="allat_pmember_id" value="<?=$sess['m_id']?>">
...
<input type="hidden" name="shop_receive_url" value="http://<?=$_SERVER['HTTP_HOST']?><?=$cfg['rootDir']?>/order/card/allat/mobile/allat_receive.php">
