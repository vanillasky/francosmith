...
<script type="text/javascript">
	...
	function approval_submit(result_cd,result_msg,enc_data) {
		...
		if( result_cd != '0000' ){
			...
			sendFm.action = "<?=$Protocol.$Host.$Port?><?=$cfg['rootDir']?>/order/card/allat/mobile/nscreen_card_return.php?page_type=<?=$page_type?>";
			sendFm.method = "post";
			sendFm.target = "_self";
			sendFm.submit();
		}
...
<input type="hidden" name="shop_receive_url" value="<?=$Protocol.$Host.$Port?><?=$cfg['rootDir']?>/order/card/allat/mobile/allat_receive.php">
