...
if (forge_order_check($_POST["order_no"],$_POST['pay_mny']) === false) {
	msg('주문 정보와 결제 정보가 맞질 않습니다. 다시 결제 바랍니다.',$order_fail_page.'?ordno='.$_POST["order_no"],'parent');
	exit();
}
...
if ($naverNcash->useyn == 'Y') {
	...
	if ($ncashResult === false) {
		msg('네이버 마일리지 사용에 실패하였습니다.', $order_fail_page.'?ordno='.$_POST["order_no"],'parent');
		exit();
	}
}
...
if($oData['step'] > 0 || $oData['vAccount'] != '' ){
	...
	go($order_end_page."?ordno=$order_no&card_nm=".$r_issuer_nm,"parent");
	exit();
}
...
if ( $TRAN_CD_NOR_PAYMENT == $tr_cd && $res_cd == "0000" )
{
	...
	----------------------------------------";
				...
				go($order_fail_page."?ordno=$order_no","parent");
	}else{
		go($order_end_page."?ordno=$order_no&card_nm=$card_nm","parent");
		}
    }else{
		if ($step == '51') {
