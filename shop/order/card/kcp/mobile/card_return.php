<?
	...
	if (forge_order_check($_POST['ordr_idxx'],$_POST['good_mny']) === false && $_POST['req_tx'] == 'pay') {
		msg('주문 정보와 결제 정보가 맞질 않습니다. 다시 결제 바랍니다.',$cfgMobileShop['mobileShopRootDir']."/ord/order_fail.php?ordno=".$_POST['ordr_idxx'],'parent');
		exit();
	}
	...
	if ($naverNcash->useyn == 'Y' && $_POST['req_tx'] != 'mod_escrow') {
		...
		if ($ncashResult === false) {
			msg('네이버 마일리지 사용에 실패하였습니다.', $cfgMobileShop['mobileShopRootDir'].'/ord/order_fail.php?ordno='.$_POST['ordr_idxx'],'parent');
			exit();
		}
	}
  ...
  if( $res_cd == "" ) //결제취소시 (KCP결제진행중 결제창을 종료시)
  {
	...
	if ( $req_tx == "pay" )
    {
		...
		else if( $res_cd == "0000" )
        {
			...
			$query = "
				...
				go($cfgMobileShop['mobileShopRootDir']."/ord/order_end.php?ordno=$ordr_idxx&card_nm=$card_name","parent");
		}
    ...
    /* ============================================================================== */
		else if ( $req_cd != "0000" )
		{
			...
			$res = false;
			go($cfgMobileShop['mobileShopRootDir']."/ord/order_fail.php?ordno=$ordr_idxx&card_nm=$card_name","parent");
		}
	}
