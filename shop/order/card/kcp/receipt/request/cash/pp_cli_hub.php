...
require_once dirname(__FILE__).'/pp_cli_hub_lib.php';
include_once dirname(__FILE__).'/../configure/site.php';
...
else if ($crdata['req_tx'] == 'mod')
{
	...
	/* = -------------------------------------------------------------------------- = */
	$ordno = $crdata['ordno'];
}else if ($crdata['req_tx'] == 'repay'){
	/* = -------------------------------------------------------------------------- = */
    $req_tx           = $crdata['req_tx'];							// 요청 종류
    $mod_type         = $data['mod_type'];                    // 변경 요청 종류
    /* = -------------------------------------------------------------------------- = */
    $res_cd           = "";                                        // 결과코드
    $res_msg          = "";                                        // 결과메시지
    $tno              = "";                                        // 거래번호
...
else if ( $req_tx == "repay" )
{
	$tx_cd = "00200000";

	$c_PayPlus->mf_set_modx_data( "tno",      $data['tno']         ); // KCP 원거래 거래번호
	$c_PayPlus->mf_set_modx_data( "mod_type", $mod_type            ); // 원거래 변경 요청 종류
	$c_PayPlus->mf_set_modx_data( "mod_ip",   $cust_ip             ); // 변경 요청자 IP
	$c_PayPlus->mf_set_modx_data( "mod_desc", $data['mod_desc'] ); // 변경 사유

	if ( $mod_type == "RN07" ) // 부분취소의 경우
	{
		$c_PayPlus->mf_set_modx_data( "mod_mny", $data['mod_mny'] ); // 취소요청금액
		$c_PayPlus->mf_set_modx_data( "rem_mny", $data['rem_mny'] ); // 취소가능잔액
	}
}
