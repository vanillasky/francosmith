...
//로그폴더 생성
$dir = dirname(__FILE__)."/../../../../log/agspay/log";
if (!is_dir($dir)) {
	@mkdir($dir, 0707);
	@chmod($dir, 0707);
}
...
<input type=hidden name=MallUrl value="<?=$Protocol.$Host.$Port?>">	<!-- ★상점URL (50) -->
...
<input type=hidden name=RtnUrl value="<? echo $Protocol.$Host.$Port.$cfg['rootDir'].'/order/card/agspay/mobile/nscreen_card_return.php?page_type='.$page_type ?>">	<!-- ★성공 URL (150) - 성공 URL은 반드시 상점의 AGS_pay_ing.php의 전체 경로로 맞춰 주시기 바랍니다. ex)http://www.allthegate.com/mall/AGS_pay_ing.php -->
<input type=hidden name=CancelUrl value="<? echo $Protocol.$Host.$Port.$cfg['rootDir'].'/mypage/mypage_orderview.php?ordno='.$_POST['ordno'] ?>">	<!-- ★취소 URL (150) - 객이 취소를 눌렀을 경우의 이동 URL 경로로 전체 경로로 맞춰 주시기 입니다. ex)http://www.allthegate.com/mall/AGS_pay_cancel.php -->
