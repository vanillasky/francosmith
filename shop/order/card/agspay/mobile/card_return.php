...
********************************************************************************/
	...
	if (forge_order_check($_POST['OrdNo'],$_POST['Amt']) === false) {
		msg('주문 정보와 결제 정보가 맞질 않습니다. 다시 결제 바랍니다.',$cfgMobileShop['mobileShopRootDir']."/ord/order_fail.php?ordno=".$_POST['OrdNo'],'parent');
		exit();
	}
	...
	if ($naverNcash->useyn == 'Y') {
		...
		{
			msg('네이버 마일리지 사용에 실패하였습니다.', $cfgMobileShop['mobileShopRootDir'].'/ord/order_fail.php?ordno='.$_POST['OrdNo'],'parent');
			exit();
		}
	}
	...
	****************************************************************************/
	...
	}else if($agspay->GetResult("rSuccYn") == "y" && $step != 51)	{	// 결제 성공
