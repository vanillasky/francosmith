...
if (forge_order_check($_GET['ordno'],$P_NOTI['P_AMT']) === false) {
	msg('주문 정보와 결제 정보가 맞질 않습니다. 다시 결제 바랍니다.',$order_fail_page.'?ordno='.$_GET['ordno'],'parent');
	exit();
}
...
if ($naverNcash->useyn == 'Y') {
	...
	if ($ncashResult === false) {
		msg('네이버 마일리지 사용에 실패하였습니다.', $order_fail_page.'?ordno='.$_GET['ordno'], 'parent');
		exit();
	}
}
...
----------------------------------------";
	...
	if(!$cancel->chk_item_stock($ordno) && $res_cstock){
		...
		$inipay->m_tid = $resData['P_TID']; // 거래번호
	...
	// 거래금액 위변조 확인
		...
		$inipay->m_uip = '127.0.0.1'; // 고정
		$inipay->m_type = 'cancel'; // 고정
		$inipay->m_msg = '거래금액 위변조 감지됨'; // 취소사유
		$inipay->startAction();
		if ($inipay->m_resultCode == '00') {
			$resData['P_STATUS'] = '01';
			$resData['P_RMESG1'] = '거래금액 위변조가 감지되어 자동취소 되었습니다.';
			$settlelog = '----------------------------------------'.
			...
			$settlelog = '----------------------------------------'.
				PHP_EOL.'결과내용 : 거래금액 위변조가 감지되었으나 자동취소 실패'.
				PHP_EOL.'PG확인 요망'.
				PHP_EOL.'----------------------------------------'.
				PHP_EOL.$settlelog;
		}
