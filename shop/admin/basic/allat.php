...
if($cfg['settlePg']!=$pg_name) $pg = array(); //pg타입체크

if ($cfg[settlePg]=="allat") $spot = "<b style='color:#ff0000;padding-left:10px'><img src=../img/btn_on_func.gif align=absmiddle></b>";
...
// allat basic 모듈 유도 버튼
$allatBtn = "<img src=\"../img/btn_allatChange.gif\" border=\"0\" align=\"absmiddle\" class=\"hand\" alt=\"All@Pay™Basic 결제 모듈로 전환\" onclick=\"allatChange()\">";
...
function get_pginfo(pgid){
	...
	{
		method: "post",
		parameters: "mode=getPginfo&pgtype=allat&pgid="+pgid,
		onComplete: function ()
		{
...
function allatChange(){
	if(confirm('All@Pay™Basic 결제 모듈로 전환 하시겠습니까?\r\n\r\n※주의 사항\r\n전환을 하시면 할부기간 수정은 올앳관리자페이지에서 하셔야 합니다.\r\n- 자세한 내용은 고도 고객센터로 연락 바랍니다.\r\n\r\n전환후 반드시 저장버튼을 눌러야 전환이 완료 됩니다.') == true){
		<?if($pg['pg-centersetting'] == 'Y'){?>
		var allatKeyName= '<?=$pg[crosskey]?>';
		<?}
		else{?>
		var allatKeyName	= document.getElementById('pgid').value;
		<?}?>
		var ajax = new Ajax.Request( "./indb.php",
		{
			method: "post",
			parameters: "mode=allatCopy&allatKeyName="+allatKeyName,
			onComplete: function ()
			{
				var req = ajax.transport;
				parent.chgifrm('allatbasic.php?changePg=allatbasic',1);
			}
		});
	}
}
...
<div id="allat_banner"><script>panel('allat_banner', 'pg');</script></div>
<form method=post action="indb.pg.php" onsubmit="return chkFormThis(this)">
<input type=hidden name=mode value="allat">
<input type=hidden name=cfg[settlePg] value="allat">
<?if($pgStatus == 'menual') {?>
	<div id=MSG01>
	<table cellpadding=1 cellspacing=0 border=0 class=small_ex>
	<tr><td>올앳에서 제공하는 신용카드,계좌이체,가상계좌,핸드폰의 결제수단을 방문자(소비자)에게 제공하기 위해서</td></tr>
	<tr><td>올앳에서 <b>메일로 받으신 올앳 PGID와 Form Key, CrossKey를 입력</b>하신후 본 페이지 하단의 저장버튼을 클릭해 주세요.</td></tr>
...
<tr>
	<td>PG사</td>
	<td class=noline><b>올앳 (All@Pay™ Plus 2.0) <?=$spot?></b> <?=$allatBtn?></td>
</tr>
<tr>
	<td>결제수단 설정</td>
	<td class=noline>
...
<tr>
	...
	<?}
		...
		<div style="clear:both" class="extext">GM_ 로 시작되는 올앳 PGID만 직접 입력 가능합니다. (단, 기존 입력값은 무방합니다)</div>
...
<table cellpadding="1" cellspacing="0" border="0" class="small_ex">
	...
	<tr><td class="small_ex_padding extext" style="font-weight: bold; color: #0174DF;">올앳 관리자에 입력할 주소 : http://쇼핑몰도메인/shop/order/card/allat/allat_notiurl.php</td></tr>
	<tr><td>③ 모바일샵용 PG ID(상점ID)를 추가로 사용하는 경우,  해당ID를 선택하고 주소를 입력하십시오(③번 등록방식과 동일)</td></tr>
	<tr><td class="small_ex_padding extext" style="font-weight: bold; color: #0174DF;">모바일샵용 PG ID 추가사용 시 입력할 주소 :  http://쇼핑몰도메인/shop/order/card/allat/mobile/allat_mobile_notiurl.php</td></tr>
...
<td align=center rowspan=2 style='border:1px solid #cccccc;padding:0 10 0 10'><a href="javascript:manual('<?=$guideUrl?>board/view.php?id=basic&no=9')"><img src="../img/icon_sample.gif" align=absmiddle></a></td>
