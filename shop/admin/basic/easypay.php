...
function chkFormThis(f){
	...
	if(!p_id.value && ret){
		p_id.focus();
		alert('이지페이 PG ID는 필수항목입니다.');
		return false;
	}

	if(!chkPgid()){
		alert('이지페이 PG ID가 올바르지 않습니다.');
		return false;
	}
	<?}?>
	return chkForm(f);
...
function openPrefix(){
	if(chkPgid()){
		alert("정상적인 이지페이 PG ID입니다.\n개별 승인 신청이 필요 없습니다.\n창을 닫고 이지페이 PG ID를 입력하세요!");
		return;
	}
...
이지페이에서 <b>메일로 받으신 이지페이 PG ID를 입력</b> 하신 후 본 페이지 하단의 저장버튼을 클릭해 주세요. <br/>
아직 이지페이와 계약을 하지 않으셨다면, <br/>
①온라인신청 하신후 ②계약서류를 우편으로 이지페이에 보내주세요. <a href="/shop/admin/basic/pg.intro.php" target="_blank" class="extext" style="color:#ffffff;">[<u>계약 상세안내</u>]</a><br/>
</td></tr>
...
<tr>
	...
	<?}?>
	<?if($pgStatus == 'menual'){?>&nbsp;&nbsp;&nbsp;<span class="extext"><b>(반드시 이지페와 계약한 결제수단만 체크하세요)</b></span><?}?>
	</td>
</tr>
<tr>
	<td>이지페이 <font color="#627dce">PG&nbsp;ID</font></td>
	<td>
	<?
	if($pgStatus == 'auto'){?>
	...
	<div style="clear:both" class="extext"><b>GD</b>로 시작되는 이지페이 PG ID만 직접 입력 가능합니다. </div>
	<?}?>
	</td>
</tr>
...
<div id=MSG02>
	<table cellpadding=1 cellspacing=0 border=0 class=small_ex>
		...
		<?}else{?>
		<tr><td><img src="../img/icon_list.gif" align="absmiddle">전자결제 서비스를 신청하면 e나무 솔루션에 PG ID가 자동으로 설정됩니다. 전자결제 신청 후 계약서류를 우편으로 이지페이에 보내주세요. <a href="pg.intro.php" target="_blank" style="color:#ffffff;">[<u>계약 상세안내</u>]</a>
		<?}?>
		...
		<tr><td><img src="../img/icon_list.gif" align="absmiddle">이지페이 PG사와 '가상계좌' 결제수단이 계약되어 있는 경우 , 이지페이 PG는 가상계좌 결제수단 사용 시 별도의 설정 없이 자동으로 입금통보를 쇼핑몰로 받을 수 있습니다.</font></td></tr>
	</table>
</div>
