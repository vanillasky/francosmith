...
function setPickedColorFromForm(obj,fromScript){
	...
	try{
		...
		alert('#FFFFFF 형식으로 입력해 주시기 바랍니다.');
		obj.value = '#FFFFFF';
		obj.focus();
		setPickedColorFromForm(obj);
		return;
	}
}
...
function initPicker(fieldName,fieldValue,depth,callFunc){
	...
	s+= "<td><div onmouseout=\"killPicker(1)\" onmouseover=\"killPicker(0)\" onclick=\"placePickerToolbar(this,"+depth+")\" style=\"padding:0px 2px 0px 2px;width:15px;height:15px;border: 1px solid #000000;cursor:pointer;background-color:"+fieldValue+";\" id=pickedColor_"+depth+"_"+donePickerInits[depth]+">color</div></td>";
	s+= "<td>&nbsp;&nbsp;<input type=\"text\" class=\"line\" style=\"border:solid 1px #cccccc;\" name=\""+fieldName+"\" id=pickedColor_"+depth+"_"+donePickerInits[depth]+"Value value=\""+fieldValue+"\" size=7 style=\"font-size:10px;\" onchange=\"setPickedColorFromForm(this)\" /></td>";
	s+= "</table>";
	document.write(s);
