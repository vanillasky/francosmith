...
list ($total) = $db->fetch("select count(*) from ".GD_BANNER." where tplSkin = '".$cfg['tplSkinTodayWork']."'"); # 총 레코드수
...
$where[] = "tplSkin = '".$cfg['tplSkinTodayWork']."'";
...
<tr>
	<td>배너위치 등록하기</td>
    <td>새롭게 배너를 추가등록하려면 먼저 <a href="javascript:popupLayer('../todayshop/codi.banner.loccd.php',780,600);"><img src="../img/btn_bangroup.gif" align="absmiddle" /></a> 부터 잡으세요. <a href="javascript:manual('<?=$guideUrl?>board/view.php?id=design&no=6')"><img src="../img/btn_bn_manual.gif" align="absmiddle" /></a></td>
</tr>
<tr>
	<td>로고/배너 선택보기</td>
	<td>
...
<tr height="25" align="center">
	...
	<td><?=$webftp->confirmImage( "../../data/skin_today/" . $cfg['tplSkinTodayWork'] . "/img/banner/" . $data['img'],200,50,"0");?></td>
	...
	<td style="padding-top:3px;"><a href="codi.banner.register.php?mode=modify&sno=<?echo($data['sno'])?>"><img src="../img/i_edit.gif" /></a></td>
...
<a href="codi.banner.register.php"><img src="../img/btn_regist_s.gif" alt="등록" align="absmiddle" /></a>
</div>
...
<tr><td><img src="../img/icon_list.gif" align="absmiddle" />투데이샵 전용 로고/배너를 제작하여 등록하실 때 일반쇼핑몰에 등록된 이미지와 중복되지 않는 이름으로 등록해주세요.</td></tr>
...
function act_allmodify(){
	...
	fmList.action = "indb.codi.banner.php?mode=allmodify";
	fmList.submit() ;
}
*/
// 수정 후 함수
...
function act_modSort( mode, code, sort ){
	fmList.action = "indb.codi.banner.php?mode=" + mode + "&code=" + code + "&sort=" + sort;
	fmList.submit() ;
}
//--></SCRIPT>
...
function act_delete(){
	...
	fmList.action = "indb.codi.banner.php?mode=delete" ;
	fmList.submit() ;
}
//--></SCRIPT>
