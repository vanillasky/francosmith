...
if ($tsPG['cfg']['settlePg'] != "dacom") $tsPG = array(); // 사용중이 아니라면 pg 정보 없앰

$tsPG['pg'] = @array_merge($_pg,$tsPG['pg']);

if($tsPG['cfg']['settlePg']!="dacom") $tsPG['pg'] = array(); //pg타입체크

if ($tsPG['cfg']['settlePg']=="dacom") $spot = "<b style='color:#ff0000;padding-left:10px'>[사용중]</b>";
...
function get_pginfo(pgid){
	...
	{
		method: "post",
		parameters: "mode=getPginfo&pgtype=dacom&todayshoppg=y&pgid="+pgid,
		onComplete: function ()
		{
...
<tr>
	...
	<div style="clear:both" class="extext">gs_ 로 시작되는 LG U+ ID만 직접 입력 가능합니다. (단, 기존 입력값은 무방합니다)</div>
	<div class="extext">gs_ 로 시작되지 않은 LG U+ ID는 개별 승인 처리후 입력되어집니다.</div>
	</td>
</tr>
<tr>
