...
if ($tsPG['cfg']['settlePg'] != "allat") $tsPG = array(); // 사용중이 아니라면 pg 정보 없앰

$tsPG['pg'] = @array_merge($_pg,$tsPG['pg']);

if($tsPG['cfg']['settlePg']!="allat") $tsPG['pg'] = array(); //pg타입체크

if ($tsPG['cfg'][settlePg]=="allat") $spot = "<b style='color:#ff0000;padding-left:10px'><img src=../img/btn_on_func.gif align=absmiddle></b>";
...
function get_pginfo(pgid){
	...
	{
		method: "post",
		parameters: "mode=getPginfo&pgtype=allat&todayshoppg=y&pgid="+pgid,
		onComplete: function ()
		{
...
<tr>
	<td>PG사</td>
	<td class=noline><b>올앳 (All@Pay™ Plus 2.0) <?=$spot?></b></td>
</tr>
<tr>
	<td>결제수단 설정</td>
	<td class=noline>
...
<tr class=ver8>
	...
	<div style="clear:both" class="extext">‘GM_,GC_’ 로 시작되는 All@Pay ID만 직접 입력 가능합니다. (단, 기존 입력값은 무방합니다)</div>
