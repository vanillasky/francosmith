...
if ( $_GET['design_file'] != '' ) { // 각 기능별 레이아웃
	...
	if ($form_type == 'file' || $form_type == 'inc'){
		if ( file_exists($tmp = dirname(__FILE__) . "/../../../data/skin_today/" . $cfg['tplSkinTodayWork'] . "/" . $_GET['design_file']) ){
			$file = @file( $tmp );
			$source = implode("",$file);
	...
	if ($form_type != 'inc')
	{
		...
		## 측면위치
		$sidefloat = array();
		$floatnm= array('left' => '왼쪽', 'right' => '오른쪽');
		foreach ($floatnm as $k => $v)
		{
			$tmp = array(
				'text' => $v,
				'value' => $k,
				'checked' => ($k == $data_file['outline_sidefloat'] ? 'checked' : ''),
				'float' => $k,
				);
			if ($form_type == 'file'){
				if ($tmp['value'] == $data_default['outline_sidefloat']){
					$tmp['text'] .= 'ⓑ';
					$tmp['value'] = 'default';
					$tmp['checked'] = ('' == $data_file['outline_sidefloat'] ? 'checked' : $tmp['checked']);
				}
			}
			$sidefloat[] = $tmp;
		}
