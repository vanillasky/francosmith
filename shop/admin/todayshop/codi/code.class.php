...
class codi {
	...
	var $design_skinToday	= array();	# 템플릿 파일 정의

	var $dir_display	= array();	# 디렉토리 출력 제어
	var $notfile		= array( '.', '..', '.bash_logout', '.bash_profile', '.bashrc', '.emacs', '.bash_history', '.bash_logout.rpmnew', '.bash_profile.rpmnew', '.bashrc.rpmnew', 'common.js', 'style.css', 'img', 'images' ); # 파일 출력 제어
	...
	function codi (){
		...
		$this->codi_path = dirname(__FILE__) . "/../../../data/skin_today/";
		$this->skin_name = $cfg['tplSkinTodayWork'];
		$this->skin_path = $this->codi_path . $this->skin_name . '/';

		@include dirname(__FILE__) . "/../../../conf/design_dir_todayshop.php";
		$this->design_dir = &$design_dir;

		@include dirname(__FILE__) . "/../../../conf/design_skin_" . $cfg['tplSkinWork'] . ".php";
		@include dirname(__FILE__) . "/../../../conf/design_skinToday_" . $cfg['tplSkinTodayWork'] . ".php";
		$this->design_skin = &$design_skin;
		$this->design_skinToday = &$design_skinToday;
	...
	function get_fileinfo( $filepath ){
		...
		$design = $this->design_skinToday[ str_replace( $this->skin_path, "", $filepath ) ];
...
class codiTree extends codi {
	...
	function resetname ($id, $text, $name){
		...
		else if ($id == 'todayshop/') $text = '페이지 디자인';
