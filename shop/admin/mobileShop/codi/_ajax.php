...
switch ( $mode ){
	...
	case "getTextarea": # Textarea
		...
		ob_start();
		### Textarea 유저 값
		if ( $_GET['body'] == 'user_body' && $cfg['tplSkinMobileWork'] != '' && $_GET['tplFile'] != '' && file_exists( $tmp = dirname(__FILE__) . "/../../../data/skin_mobile/" . $cfg['tplSkinMobileWork'] . $_GET['tplFile'] ) ){
			$file = @file( $tmp );
			$output = implode("",$file);
		}

		### Textarea 원본 값
		if ( $_GET['body'] == 'base_body' &&  $cfg['tplSkinMobileWork'] != '' && $_GET['tplFile'] != '' && file_exists( $tmp = dirname(__FILE__) . "/../../../skin_m_ori/" . $cfg['tplSkinMobileWork'] . $_GET['tplFile'] ) ){
			$file = file( $tmp );
			$output = implode("",$file);
		}
		ob_end_clean();
