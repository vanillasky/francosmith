...
{
	$tmp = dirname(__FILE__) . "/../../../../m/" . str_replace(".htm", ".php", $_GET['design_file']);
	...
	if (preg_match("/(board\/|goods\/list\/|goods\/goods_event\.htm|ord\/|_myBox\.htm|_sub_header\.htm)/i", $_GET['design_file']));
		...
		$isPreview = true;
		$real_linkurl = "./../../../../m/" . str_replace(".htm", ".php", $_GET['design_file'])."?tplSkin=".$cfg['tplSkinMobileWork'];
		...
		$isPreview = true;
		$real_linkurl = "./../../../../m/html.php?htmid={$_GET['design_file']}&tplSkin=".$cfg['tplSkinMobileWork'];
	}
}
	...
	foreach($design_skin as $key => $val) {
		...
		foreach($val as $key2 => $val2) {
			...
			if ($val2 == $_GET['design_file']) {
				$isPreview = true;
				$real_linkurl = "./../../../../m/".$val['linkurl']."?tplSkin=".$cfg['tplSkinMobileWork'];
				break 2;
			}
		}
	}
	...
	$isPreview = true;
	$real_linkurl = "./../../../../m/index.php?tplSkin=".$cfg['tplSkinMobileWork'];
}
?>
<a name="codi_info"></a>
...
<?=gen_design_history_tag('skin_mobile', $cfg['tplSkinMobileWork'], $_GET['design_file']); ?>
