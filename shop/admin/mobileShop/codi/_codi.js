...
function file_batch()
{
	...
	fobj.method = "post";
	fobj.action = "../mobileShop/codi/indb.php?mode=batch";
	fobj.submit();
}
...
function designcodeMove(sobj)
{
	...
	if (path != 'noprint' && path != ''){
		document.location.href='../mobileShop/iframe.codi.php?design_file=' + path;
	}
...
DCSM = {
	...
	chk: function (fobj)
	{
		...
		document.fm.action = '../mobileShop/codi/indb.php?mode=saveas&design_file=' + fobj.dir_name.value + fobj.file_name.value + fobj.file_ext.value;
		document.fm.submit();
	...
	file_check: function ()
	{
		...
		var urlStr = "../mobileShop/codi/_ajax.php?mode=chkFile&dir_name=" + fobj.dir_name.value + "&file_name=" + fobj.file_name.value + "&file_ext=" + fobj.file_ext.value + "&dummy=" + new Date().getTime();
		var ajax = new Ajax.Request( urlStr,
		{
			method: "get",
			onComplete: function ()
...
function file_del(design_file)
{
	...
	fobj.method = "post";
	fobj.action = "../mobileShop/codi/indb.php?mode=del&design_file=" + design_file;
	fobj.submit();
}
...
DCTM = {
	...
	write: function (t_name, t_width, t_rows, t_property, tplFile)
	{
		...
		<input type="button" class="icon" style="background:url(../img/codi/bu_06.gif);width:32px;" title="기본" onclick="DCTM.row_direct( ' + t_rows + ' );">\
				<input type="button" class="icon" style="background:url(../img/codi/bu_07.gif);width:58px;" title="약도만들기" onclick="javascript:alert(\'다음 약도만들기 API 서비스가 종료되었습니다. 다음지도의 약도만들기 서비스를 이용하여 우리회사의 약도를 넣어주세요\');window.open(\'http://blog.daum.net/daummaps/482\');">\
		<br>\
	...
	source: function (tplFile, body)
	{
		...
		var urlStr = "../mobileShop/codi/_ajax.php?mode=getTextarea&body=" + body + "&tplFile=" + tplFile + "&dummy=" + new Date().getTime();
		var ajax = new Ajax.Request( urlStr,
		{
			method: "get",
			onComplete: function ()
	...
	stylesheet: function ()
	{
		var hrefStr = '../mobileShop/iframe.css.php';
...
DCRM = {
	...
	{
		var urlStr = "../mobileShop/codi/_ajax.php?mode=getReplacecode&design_file=" + design_file;
		var ajax = new Ajax.Request( urlStr,
		{
			method: "get",
			onComplete: function ()
	...
	clip: function (code)
	{
		if ( window.clipboardData ) {
			...
			temp = prompt("코드를 클립보드로 복사(Ctrl+C) 하시고. \n원하는 곳에 붙여넣기(Ctrl+V)를 하시면 됩니다~", code);
		}
	}
}

document.write("<script type='text/javascript' src='../design/codi/_codi_ex.js'></script>");
