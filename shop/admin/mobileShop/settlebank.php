...
if ($set['use_mobile'][o]) $checked[o] = "checked";
...
function chkSettleKind(){
	...
	{
		sk = document.getElementsByName('set[use_mobile]['+arr[i]+']')[0];
...
function notChange()
{
	alert('PG중앙화사용 PG는 해당 결제 수단 설정을 변경할수없습니다.');	
}
...
<tr>
	<td>결제수단 설정</td>
	<td class=noline>
<?	$use_moblie = array('c'=>'신용카드','v'=>'가상계좌','h'=>'휴대폰');
	foreach( $use_moblie as $k => $v) {
		if($set[use_mobile_ck][$k] == 'on'){
			echo "<input type=checkbox name='set[use_mobile][$k]' ".$checked[$k]." >"; 
			echo $use_moblie[$k];
		} else {
			echo "<input type=checkbox name='set[use_mobile][$k]' ".$checked[$k]." style='background:#e3e3e3;' onclick='notChange();return false;'>";
			echo '<font style="background-color:#e3e3e3;margin:20px 0;">'.$use_moblie[$k].'</font>';
		}
	}
?>
