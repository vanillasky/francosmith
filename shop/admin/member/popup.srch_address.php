...
<tr>
	<td>
	<select name="stype">
	<option value="sms_name" <?=$selected['stype']['sms_name']?>> 이름 </option>
	<option value="sms_mobile" <?=$selected['stype']['sms_mobile']?>> 핸폰번호 </option>
	</select>
	</td>
	<td>
...
<tr height=24>
	...
	<td><font class=small color=262626><?=$data['sms_grp']?></td>
	<td><font class=ver8 color=262626><?=$data['sms_name']?></td>
	<td><a href="javascript:parent.sendNumber('<?=$data['sms_mobile']?>')"><font class="ver8 el-mobile-str"><?=$data['sms_mobile']?></font></a><?php if($smsFailCheck === true){ ?><font class="ver8 el-mobile-str" color="red"> SMS 발송실패 번호</font> <?php } ?></td>
</tr>
<? } ?>
</tbody>
</table>
