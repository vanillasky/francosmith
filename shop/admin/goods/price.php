...
header('location: ./adm_goods_manage_price.php?'.$_SERVER['QUERY_STRING']);
exit;
$location = "상품관리 > 빠른 가격수정";
include "../_header.php";
...
$selected[dtprice][$_GET[dtprice]] = "selected";
$selected[pmmark][$_GET[pmmark]] = "selected";
$selected[pmtype][$_GET[pmtype]] = "selected";
$selected[tgprice][$_GET[tgprice]] = "selected";
...
if (in_array($_GET[indicate], array('search', 'main', 'event')) === true){
	...
	$pg->field = "a.goodsno,a.goodsnm,a.open,a.img_s,b.*";
...
<div class="title title_top">빠른 가격수정<span>등록하신 상품가격을 일괄 수정할 수 있습니다.</span> <a href="javascript:manual('<?=$guideUrl?>board/view.php?id=product&no=13')"><img src="../img/btn_q.gif" border=0 align=absmiddle hspace=2></a></div>

<!-- 상품출력조건 : start -->
<form name=frmList onsubmit="return chkForm(this)">

<div style="padding:10 0 5 5"><font class="def1" color="#000000"><b><font size="3">①</font> 먼저 아래에서 가격수정할 상품을 검색합니다. <font class=extext>(아래 3가지 방식중 한가지를 선택하여 검색)</font></b></font></div>
...
<tr class=rndbg>
	...
	<th align=right style="padding-right:3"><font class=small1><b>적립금</th>
...
<tr>
	...
	<td align=right style="padding-right:10px" nowrap><font class="ver8" color="#0074BA"><b><?=number_format($data[consumer])?></b></font></td>
	<td align=right style="padding-right:10px" nowrap><font class="ver8" color="#0074BA"><b><?=number_format($data[price])?></b></font></td>
...
<div style="padding:20 0 5 5"><font class="def1" color="#000000"><b><font size="3">②</font> 위 상품리스트에 있는 상품의 가격을, 아래 조건을 적용하여 일괄수정합니다. <font class=extext>(신중하게 설정하고 수정하세요)</b></font></div>
<table class=tb>
<col class=cellC><col class=cellL>
<tr>
	<td>가격조건설정</td>
	...
	<option value="consumer" <?=$selected[dtprice][consumer]?>>정가</option>
	<option value="supply" <?=$selected[dtprice][supply]?>>매입가</option>
	</select> 에서
	<input type="text" name="pmnum" value="<?=$_GET[pmnum]?>" required label="계산수치" option="/^[0-9]+(\.[0-9]){0,1}$/" size="4" class="rline" style="width:60px">
	<select name="pmmark">
	...
	<option value="consumer" <?=$selected[tgprice][consumer]?>>정가</option>
...
<tr><td><img src="../img/icon_list.gif" align="absmiddle">일괄수정 할 상품을 검색 후 상품가격을 일괄처리 조건에 맞춰 적용합니다.</td></tr>
<tr><td><img src="../img/icon_list.gif" align="absmiddle">[주의1] 일괄수정 이후에는 <b>이전상태로 복원이 안되므로 신중하게 변경하시기 바랍니다.</b></td></tr>
<tr><td><img src="../img/icon_list.gif" align="absmiddle">[주의2] 서버 부하등 안정적인 서비스를 위해서 수정할 상품이 많은 경우에는 검색된 상품 전체수정은 피하시기 바랍니다.</td></tr>
<tr><td><img src="../img/icon_list.gif" align="absmiddle">할인/할증율(%) : 소수점 첫째자리까지 입력하실 수 있습니다. [예] 5.5% 할인(가능), 5.5% 할증(가능), 5.55% 할인(불가능)</td></tr>
<tr><td style="padding-top:4"><img src="../img/icon_list.gif" align="absmiddle"><b>[가격수정 예제]</b></td></tr>
