<?php
$location = '네이버체크아웃 4.0 > 주문리스트';
include '../_header.php';
include "../../lib/page.class.php";

function _getRemainDay($duedate) {
...
// 쿼리 실행
	...
	$pg->field = "
		...
		IF ( C.ProductOrderID IS NOT NULL, C.HoldbackStatus ,
		IF ( R.ProductOrderID IS NOT NULL, R.HoldbackStatus ,
		IF ( E.ProductOrderID IS NOT NULL, E.HoldbackStatus ,
			NULL
		))) AS HoldbackStatus,

		IF ( C.ProductOrderID IS NOT NULL, C.HoldbackReason ,
		IF ( R.ProductOrderID IS NOT NULL, R.HoldbackReason ,
		IF ( E.ProductOrderID IS NOT NULL, E.HoldbackReason ,
			NULL
		))) AS HoldbackReason
...
<div class="title title_top">전체주문조회 <span>네이버체크아웃의 전체 주문내역을 확인/관리하는 페이지 입니다.</span></div>
...
<div style="margin:15px 0 5px 0;_border:1px solid #fff">
	...
	<div class="fl">
		...
		<? if ($_GET['ProductOrderStatus'] == '입금확인' ) { ?>
		<button class="default-btn" onClick="fnPlaceProductOrder();return false;">발주확인</button>
		<button class="default-btn" onClick="fnDelayProductOrder();return false;">발송지연</button>
		<button class="default-btn" onClick="fnCancelSale();return false;">판매취소</button>

		<? } elseif ($_GET['ProductOrderStatus'] == '배송준비중' ) { ?>
		<button class="default-btn" onClick="alert('발송처리는 배송준비중 리스트를 이용하세요.');return false;">발송처리</button>
		<button class="default-btn" onClick="fnDelayProductOrder();return false;">발송지연</button>

		<? } elseif ($_GET['ProductOrderStatus'] == '배송중' ) { ?>
		<button class="default-btn" onClick="fnRequestReturn();return false;">반품신청</button>

		<? } elseif ($_GET['ProductOrderStatus'] == '배송완료' ) { ?>
		<button class="default-btn" onClick="fnRequestReturn();return false;">반품신청</button>

		<? } elseif ($_GET['ProductOrderStatus'] == '취소요청' ) { ?>
		<button class="default-btn" onClick="fnApproveCancelApplication();return false;">취소요청승인</button>

		<? } elseif ($_GET['ProductOrderStatus'] == '반품요청' || $_GET['ProductOrderStatus'] == '반품수거중' ) { ?>
		<button class="default-btn" onClick="fnApproveReturnApplication();return false;">반품요청승인</button>

		<? } elseif ($_GET['ProductOrderStatus'] == '교환요청' || $_GET['ProductOrderStatus'] == '교환수거중') { ?>
		<button class="default-btn" onClick="fnApproveExchangeApplication();return false;">교환요청승인</button>
		<button class="default-btn" onClick="fnApproveExchangeApplication();return false;">교환수거완료</button>
		<? } elseif ($_GET['ProductOrderStatus'] == '교환수거완료' ) { ?>
		<button class="default-btn" onClick="fnApproveExchangeApplication();return false;">교환요청승인</button>
		<? } elseif ($_GET['ProductOrderStatus'] == '교환배송준비중' ) { ?>
...
<form name="frmNaverCheckout" method="post" target="processLayerForm">
	...
	<col width="35"><col width="40"><col width="100"><col width="140"><col><col width="70"><col width="70"><col width="50"><col width="55"><col width="50">
	...
	while ($row = $db->fetch($rs,1)) {
		...
		$disabled = $row['HoldbackStatus'] == 'HOLDBACK' && $row['HoldbackReason'] == 'PURCHASER_CONFIRM_NEED' ? 'disabled' : '';
	...
	<tr height="25" bgcolor="#ffffff" bg="#ffffff" align="center">
		...
		<td align="left">
			...
			<?=(!empty($row['DelayedDispatchReason']) ? '<span class="small1">(발송지연)</span>' : '')?>
		</td>
		<td align="left">
...
<tr><td><img src="../img/icon_list.gif" align="absmiddle"/>주문상태별로 주문내역을 확인 하고 처리상태를 변경, 관리 하실 수 있습니다.</td></tr>
</table>
</div>
