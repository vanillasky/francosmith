...
$checked['processtype'][$_GET['processtype']] = 'checked';
...
// 쿼리 생성
	// 셀렉트 필드.
		$_SQL['FIELD'] = "
			...
			b.m_id, b.m_no,
			G.goodsno, G.goodsnm,
			TG.tgsno, TG.goodsno, TG.encor, TG.visible, TG.startdt, TG.enddt, TG.regdt, TG.buyercnt, TG.fakestock, TG.limit_ea, TG.goodstype, TG.processtype, 
			IF (
				NOW() < TG.startdt,
				1,	/* 판매대기 */
				IF (
					((NOW() >= TG.startdt AND NOW() <= TG.enddt) OR (TG.enddt IS NULL)) AND (G.usestock<>'o' OR G.totstock>0),
					2,	/* 판매중 */
					IF (
						TG.limit_ea <> 0 AND TG.buyercnt+TG.fakestock < TG.limit_ea,
						3,	/* 판매실패 */
						4	/* 판매완료 = 판매종료 */
					)
				)
	...
	// WHERE 절 (각 배열 항목당 and 연산임)
		...
		if ($_value = trim($_GET[stats])) {
			switch ($_value) {
				case 2:
					$_SQL['WHERE'][] = " (((NOW() >= TG.startdt AND NOW() <= TG.enddt) OR (TG.enddt IS NULL)) AND (G.usestock<>'o' OR G.totstock>0)) ";	// 판매중
					break;
				case 3:
					$_SQL['WHERE'][] = " (NOW() >= TG.enddt OR (G.usestock='o' AND G.totstock=0)) AND (TG.limit_ea <> 0 AND TG.buyercnt+TG.fakestock < TG.limit_ea) ";	// 판매 실패
					break;
				case 4:
					$_SQL['WHERE'][] = " (NOW() >= TG.enddt OR (G.usestock='o' AND G.totstock=0)) AND (TG.limit_ea = 0 OR TG.buyercnt+TG.fakestock >= TG.limit_ea) ";	// 판매종료(=판매완료)
					break;
			}
		}
...
function chkColor(obj)
{
	...
	if (obj.options[obj.selectedIndex].value!=obj.getAttribute('index')){
		row.style.background = "#F9FFF0";
		obj.parentNode.getElementsByTagName('input')[0].checked = true;
	} else {
		row.style.background = "#FFFFFF";
		obj.parentNode.getElementsByTagName('input')[0].checked = false;
	}
}
*/
...
<div class="title title_top" style="position:relative;padding-bottom:15px">투데이샵 주문 리스트<span>투데이샵의 주문을 확인하고 주문상태를 변경합니다</span>
...
<form>
	...
	<tr>
		<td><font class=small1>상품유형</td>
		<td class="noline small1" style="color:5C5C5C; margin-right:20px;">
		...
		<td><font class=small1>처리시점</td>
		<td class="noline small1" style="color:5C5C5C; margin-right:20px;">
		<label><input type="radio" name="processtype" value="" <?=$checked['processtype']['']?>>전체</label>
		<label><input type="radio" name="processtype" value="i" <?=$checked['processtype']['i']?>>즉시</label>
		<label><input type="radio" name="processtype" value="b" <?=$checked['processtype']['b']?>>일괄</label>
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td colspan=3 class="noline small1" style="color:5C5C5C; margin-right:20px;">
...
<!--
	<option value="0">주문접수 처리
	<option value="1">입금확인 처리
	<option value="2">배송준비중 처리
	<option value="3">배송중 처리
	<option value="4">배송완료 처리
-->
...
<!-- 상품별 리스트-->
	...
	<tr align=center bg="">
		...
		<td class="noline small4" valign="top" style="padding:9px"><?=($item['goodstype']=='coupon')? '쿠폰':'실물'?><br /> / <br /><?=($item['processtype']=='i')? '즉시':'일괄'?></td>
		<td class="noline small4" valign="top" style="padding:9px"><?=$item['startdt']?><br/> ~ <br/><?=$item['enddt']?></td>
		<td class="noline small4" valign="top" style="padding:9px"><?=number_format($item['buyercnt']+$item['fakestock'])?> / <?=($item['limit_ea'] > 0) ? number_format($item['limit_ea']) : '미지정'?></td>
		...
		<td colspan="8">
			...
			?>
			<tr height="30">
				<td class="noline small4" valign="top" style="padding-top:7px;"><?=($pg->idx--)?></td>
	...
	<tr><td colspan=13 bgcolor=E4E4E4></td></tr>
	<? } ?>
	<?
		$cnt = $pr * ($idx+1);
		$s = $idx_grp - $cnt;
	?>
	<tr>
		<td align=right height=30 colspan=13 style=padding-right:8>합계: <!--(<?=$cnt?>건)--> <font class=ver9><b><?=number_format($grp[settleprice][$preStepMsg])?></font>원</b></td>
		<td></td>
	</tr>
	<tr bgcolor=#f7f7f7 height=30>
		<td colspan=13 align=right style=padding-right:8>전체합계 : <span class=ver9><b><?=number_format(@array_sum($grp[settleprice]))?>원</b></span></td>
		<td></td>
	</tr>
...
<!-- 주문별 리스트-->
	...
	<tr height="35" align=center bg="">
		<td class="noline small4"><?=($pg->idx--)?></td>
		<td class=small4><font class=ver81 color=616161><?=substr($row[orddt],0,-3)?></font></td>
		<td class=small4>
			<a href="view.php?ordno=<?=$row[ordno]?>"><font class=ver81 color=0074BA><b><?=$row[ordno]?></b></font></a>
			<a href="javascript:popup('popup.order.php?ordno=<?=$row[ordno]?>',800,600)"><img src="../img/btn_newwindow.gif" border=0 align=absmiddle></a>
		</td>
		<td class="noline small4"><?=$item[goodsnm]?></td>
		<td class="noline small4"><?=($item['goodstype']=='coupon')? '쿠폰':'실물'?> / <?=($item['processtype']=='i')? '즉시':'일괄'?></td>
