...
/*
 * 검색조건별 네이버 페이 주문 노출 설정 START
 * 2015-10-07
*/
//제휴처주문
if(empty($search['chk_inflow']) === false && is_array($search['chk_inflow']) && array_search('naverCheckout', $search['chk_inflow']) === false) return false;

//에스크로
if($search['escrowyn'] === 'y') return false;

//전자보증보험
if($search['eggyn'] === 'f' || $search['eggyn'] === 'y') return false;

//투데이샵
if($search['todaygoods'] === 'y') return false;

//모바일샵
if($search['mobilepay'] === 'y') return false;

//결제방법
if($search['couponyn'] == 1 || $search['cashreceipt'] == 1 || $search['cbyn'] === 'Y' || $search['aboutcoupon'] == 1 || $search['payco'] == 1) return false;

//접수유형(수기접수)
if($search['sugi'] === 'Y') return false;
