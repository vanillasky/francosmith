<?php
/*********************************************************
* 파일명     :  paycoCancelVbank.php
* 프로그램명 :  페이코 결제 전체취소(가상계좌/계좌이체)
**********************************************************/
include "../lib.php";
...
if($get_ordno) {
	if($get_sno) {
		...
		*/
		...
		$real_settle_price = $order->getCancelCompletedRealSettleAmount();//이나무 실제남은 금액 - 취소된 배송비
		...
		if($cash_bool === true && $point_bool === false) {
			...
			if($payco_point > $arr_order_data['prn_payco_use_point']) $payco_point = $arr_order_data['prn_payco_use_point'];
		...
		if($cancel_no_yn !== true) {
			...
			echo('<script>location.href = "./paycoCancelVbank.php?ordno='.$get_ordno.'&sno='.$cancel_no['sno'].'";</script>');
			exit;
		}
		else {
			echo('<script>alert("이미 취소접수/완료된 주문상품이 있습니다.\n환불접수리스트에서 처리해 주시기 바랍니다.");parent.location.reload();</script>');
			exit;
		}
	}
...
<script>
	...
	// 페이코 포인트 환불가능여부(m = 0) 확인 및 환불처리(m = 1)
