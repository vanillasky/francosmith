...
<div class="title title_top">네이버 체크아웃 판매취소 처리</div>
...
foreach($arOrderNo as $orderNo) {
	...
	echo '주문번호 '.$result[0]['ORDER_OrderID'].'에 대한 판매취소처리 중입니다<br>';
	flush();
	if($naverCheckoutAPI->CancelSale($orderNo,$CancelReason,$CancelReasonDetail)) {
		echo '판매취소처리 정상적으로 처리하였습니다<br><br>';
	}
	else {
		echo '판매취소 작업 중 오류가 발생했습니다<br>'.$naverCheckoutAPI->error.'<br><br>';
	}
	flush();
}
echo '완료되었습니다';
