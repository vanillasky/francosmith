...
if($set[delivery][basis])
{
	...
	<?
			...
			if ($_GET['ordno']) {
				$data = $db->fetch("select deliveryno, deliverycode  from ".GD_ORDER." where ordno='$_GET[ordno]' limit 1");
				$code = $data[deliveryno];
				$dlvno = $data[deliverycode];
			}
			else {

				$dlvno = $_GET['dlvno'];

				// 택배사 매칭 맵을 이용, 이나무 코드로 변경
				if ($_GET['channel'] == 'enamoo') {
					$code = $_GET['code'];
				}
				else {
					$code = $integrate_cfg['dlv_company']['map'][$_GET['channel']][$_GET['code']];
				}
			}

			if (!$code) {
				msg('배송추적이 불가능한 택배사입니다.','close');
				exit;
			}

			displayDelivery($code,$dlvno);
...
echo "<div style=\"padding:20px 0 10px 0\" align=center>
<a href=\"javascript:window.close()\"><img src=\"../img/btn_confirm_s.gif\"></a></div>";
?>