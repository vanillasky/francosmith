...
// 검색절 만듦
	...
	#1. 주문상태
		$arWhere[] = '('.$checkout_message_schema['extra_productOrderStatusType']['배송중'].')';
...
<div class="title title_top">배송중리스트 <span>발송 처리된 배송중인 주문건들을 확인/관리 하는 페이지 입니다.</span></div>
...
<form name="frmNaverCheckout" method="post" target="processLayerForm">
	...
	<col width="25"><col width="35"><col width="100"><col width="100"><col width="140"><col><col width="90"><col width="60"><col width="130"><col width="50"><col width="80">
	...
	?>
		<tr height="25" bgcolor="#ffffff" bg="#ffffff" align="center">
			<td class="noline">
				<input type="checkbox" name="OrderID[<?=$idx?>]" value="<?=$row['OrderID']?>" class="el-OrderID {ProductOrderStatus:'<?=$row['ProductOrderStatus']?>',ClaimType:'<?=$row['ClaimType']?>',ClaimStatus:'<?=$row['ClaimStatus']?>',PlaceOrderStatus:'<?=$row['PlaceOrderStatus']?>',HoldbackStatus:'<?=$row['HoldbackStatus']?>',HoldbackReason:'<?=$row['HoldbackReason']?>'}" onclick="iciSelect(this)"/>
			...
			<td><span class="ver81" style="color:#616161"><?=substr($row[OrderDate],0,-3)?></font></td>
			<td><span class="ver81" style="color:#616161"><?=substr($row[SendDate],0,-3)?></font></td>
			<td align="left">
			...
			<td class="small4"><?=$row['ShippingAddressBaseAddress']?> <?=$row['ShippingAddressDetailedAddress']?> <?=$row['ShippingAddressTel1']?></td>
			<td class="small4"><?=$checkout_message_schema['deliveryMethodType'][$row['DeliveryMethod']]?></td>
			<td class="ver81">
				<? if ($row['DeliveryMethod'] == 'DELIVERY' || $row['DeliveryMethod'] == 'GDFW_ISSUE_SVC') { ?>
				<a href="javascript:void(0);" onClick="fnDeliveryTrace('checkout','<?=$row['DeliveryCompany']?>','<?=$row['TrackingNumber']?>');"><img src="../img/btn_int_order_tracking.gif"></a> <br>
				<?=$checkout_message_schema['deliveryCompanyType'][$row['DeliveryCompany']]?> / <?=$row['TrackingNumber']?>
				<? } else { ?>&nbsp;<? } ?>
			</td>
		</tr>
...
<tr><td><img src="../img/icon_list.gif" align="absmiddle"/>발송 처리된 주문내역을 확인하고, 해당 주문건의 배송 현황을 확인합니다.</td></tr>
<tr><td>&nbsp;</td></tr>
<tr><td><img src="../img/icon_list.gif" align="absmiddle"/>배송추적 :  배송중인 주문건의 배송현황을 확인할 수 있습니다.</td></tr>
<tr><td><img src="../img/icon_list.gif" align="absmiddle"/>반품신청 :  구매자를 통하여 반품 요청이 들어온 주문건을 반품신청 처리 합니다.</td></tr>
</table>
</div>
