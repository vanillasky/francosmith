...
<form name=frmList>
<div class="title title_top">일반발행요청리스트<span>구매고객이 신청한 세금계산서를 조회하고 승인처리합니다</span> <a href="javascript:manual('<?=$guideUrl?>board/view.php?id=order&no=8')"><img src="../img/btn_q.gif" border=0 hspace=2 align=absmiddle></a></div>
<table class=tb>
...
while ($data=$db->fetch($res)){
	...
	$step = $r_stepi[$order[step]][$order[step2]];
	$cashMsg = ($order[cashreceipt])?"현금영수증발행":"";

	$realSettlePrice = 0;
	$realSettlePrice = $order->getRealPrnSettleAmount();

	### 신청자
	if ( !$data[m_no] ) $namestr = $order[nameOrder];
	...
	if($data[goodsnm]) $data[goodsnm] = htmlspecialchars($data[goodsnm]);
	if($data[company]) $data[company] = htmlspecialchars($data[company]);
	?>
<tr height=25 align="center">
	<td rowspan=2 class=noline><input type=checkbox name=chk[] value="<?=$data[sno]?>" subject="주문번호 <?=$data[ordno]?>" cashreceipt="<?=$order[cashreceipt]?>" onclick="TAM.iciSelect(this)"></td>
	...
	<td><input type=text name="issuedate[<?=$data[sno]?>]" value="<?=$data[issuedate]?>" size=10 maxlength=10 style="text-align:center;" class=line></td>
...
<tr height=25 align="center">
	...
	<td><?=number_format($realSettlePrice)?></td>
...
<img src="../img/btn_alldelet_s.gif" alt="선택삭제" border="0" align='absmiddle' style="cursor:hand" <?if ( $pg->recode[total] != 0 ){?>onclick="javaScript:TAM.act_delete();"<?}else{?>onclick="javascript:alert( '데이타가 존재하지 않습니다.' );"<?}?>>
</div>
...
<A HREF="javascript:TAM.act_agree();"><img src="../img/btn_tax_app.gif" alt="일반발행승인" border=0 align=absmiddle></a>
</div>
...
<tr><td><img src="../img/icon_list.gif" align="absmiddle">일반세금계산서 발행승인
<ol type="a" style="margin:0px 0px 0px 40px;">
<li>일반세금계산서(종이 세금계산서)를 발행합니다.</li>
<li>발행신청을 승인하면 쇼핑몰에서 세금계산서를 출력 받을 수 있으며 승인 후에는 세금계산서 정보를 수정할 수 없습니다.</li>
</ol>
</td></tr>
</table>
</div>
