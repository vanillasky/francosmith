<?
$location = "주문관리 > 단계별 주문관리 > 배송준비중 리스트";
include "../_header.php";
...
// 쿼리 실행
	...
	if ($_GET['mode'] == 'ordno') {
		...
		while ($row = $db->fetch($res,1)) {
			$arList[$row['ordno']][] = $row;
		}

		// 총 주문건수
		list($total) = $db->fetch("SELECT COUNT(DISTINCT o.ordno) FROM ".$db_table." WHERE (o.ord_status = 2)");
		list($search) = $db->fetch("SELECT COUNT(DISTINCT o.ordno) FROM ".$db_table." WHERE ".implode(' AND ',$arWhere));
		...
		";
		$pg->setQuery($db_table,$arWhere,$orderby,'GROUP BY o.channel, oi.goodsnm, oi.`option`');
		$pg->exec();
		$res = $db->query($pg->query);
	...
	$_default_dlv_company = array();
	$_default_dlv_company['enamoo'] = null;	// 맨 위의 택배사가 기본값.
	$_default_dlv_company['ipay'] = $auctionIpayCfg['default_dlv_company'];
	$_default_dlv_company['checkout'] = $checkoutCfg['default_dlv_company'];
	$_default_dlv_company['shople'] = $shopleCfg['dlv_company'];
?>
<script type="text/javascript" src="./integrate_order_common.js"></script>

<div class="title title_top" style="position:relative;padding-bottom:15px">배송준비중 리스트 <a href="javascript:manual('<?=$guideUrl?>board/view.php?id=order&no=27')"><img src="../img/btn_q.gif" border="0" hspace="2" align="absmiddle"/></a></div>
<?
...
<form name="frmSearch" id="frmSearch" method="get" action="">
	...
	<tr>
		...
		<td colspan="3">
			...
			&nbsp; &nbsp; &nbsp;
			<input type="checkbox" name="part_cancel" value="y" <?=frmChecked('y',$_GET['part_cancel'])?> />취소상품 포함
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		총 <b><?=$search?></b>건 검색, <b><?=$pg->page[now]?></b> of <?=$pg->page[total]?> Pages
		</td>
	</tr>
	</table>
	<? } ?>
	...
	<tr>
		...
		<a href="?<?=$_paging_query?>&mode=goods"><img src="../img/btn_int_order_list_by_goods<?=$_GET['mode'] == 'goods' ? '_on' : ''?>.gif"></a>
		</td>
		<td align="right">

		<select name="sort" onchange="this.form.submit();">
			<option value="oi.goodsnm desc" <?=$_GET['sort'] == 'oi.goodsnm desc' ? 'selected' : '' ?>>상품명순↑</option>
			<option value="oi.goodsnm asc" <?=$_GET['sort'] == 'oi.goodsnm asc' ? 'selected' : '' ?>>상품명순↓</option>
...
<form name="frmList" method="post" action="indb.php" id="frmList">
	...
	<?
	if ($_GET['mode'] == 'ordno')
		include(dirname(__FILE__).'/list.step2.inc.ordno.php');
	else
		include(dirname(__FILE__).'/list.step2.inc.goods.php');
	?>
	...
	<div>
		...
		<a href="javascript:void(0);" onClick="fnSetOrder(3);"><img src="../img/btn_int_order_delivery.gif" align="absmiddle"></a>
		<a href="javascript:void(0);" onClick="fnSetOrder(1);"><img src="../img/btn_int_order_payconfirm.gif" align="absmiddle"></a>
	</div>
...
<tr><td><img src="../img/icon_list.gif" align="absmiddle"/>배송준비중 상태의 주문건에 대한 리스트입니다.</td></tr>
<tr><td><img src="../img/icon_list.gif" align="absmiddle"/>검색매뉴 조건에 맞게 배송준비중 상태의 주문서를 조회할 수 있습니다.</td></tr>
<tr><td><img src="../img/icon_list.gif" align="absmiddle"/>조회한 리스트를 주문번호별, 상품별로 확인할 수 있습니다.</td></tr>
<tr><td>&nbsp;</td></tr>
<tr><td><img src="../img/icon_list.gif" align="absmiddle"/>주문번호별로 보기</td></tr>
<tr><td><img src="../img/icon_list.gif" align="absmiddle"/>&nbsp;&nbsp;- 주문번호별로 상품정보 및 배송정보등 배송준비에 필요한 주문정보를 리스트에서 확인할 수 있습니다.</td></tr>
<tr><td><img src="../img/icon_list.gif" align="absmiddle"/>&nbsp;&nbsp;- 주문번호별로 송장번호를 리스트에서 개별 등록 할 수 있습니다.</td></tr>
<tr><td>&nbsp;</td></tr>
<tr><td><img src="../img/icon_list.gif" align="absmiddle"/>상품별로 보기</td></tr>
<tr><td><img src="../img/icon_list.gif" align="absmiddle"/>&nbsp;&nbsp;- 조회된 배송준비중 리스트를 상품의 옵션별로 확인할 수 있습니다.</td></tr>
<tr><td><img src="../img/icon_list.gif" align="absmiddle"/>&nbsp;&nbsp;- 중복되는 상품을 각각 출력하지 않습니다.</td></tr>
<tr><td>&nbsp;</td></tr>
<tr><td><img src="../img/icon_list.gif" align="absmiddle"/>배송중 처리</td></tr>
<tr><td><img src="../img/icon_list.gif" align="absmiddle"/>&nbsp;&nbsp;- 배송준비된 주문건들을 확인하고 택배사에 인계된 주문건들을 선택하여 배송중으로 주문상태를 변경 합니다.</td></tr>
<tr><td>&nbsp;</td></tr>
<tr><td><img src="../img/icon_list.gif" align="absmiddle"/>입금확인으로 변경</td></tr>
<tr><td><img src="../img/icon_list.gif" align="absmiddle"/>&nbsp;&nbsp;- 배송준비중인 주문건중 배송준비가 불가능하거나 잘못처리된 주문건을 선택하여 다시 입금확인으로 주문상태를 변경합니다.</td></tr>
<tr><td><img src="../img/icon_list.gif" align="absmiddle"/>&nbsp;&nbsp;※ 네이버체크아웃, 쇼플, 옥션iPay  주문건은 입금확인으로 주문상태 변경이 불가능합니다.</td></tr>
</table>
</div>
