...
// 검색절 만듦
	...
	#1. 주문상태
		if ($_GET['ProductOrderStatus'] > -1) {
			...
			$arWhere[] = '('.$checkout_message_schema['extra_productOrderStatusType']['반품전체'].')';
		}
...
// 쿼리 실행
	$db_table = "".GD_NAVERCHECKOUT_ORDERINFO." AS O
		...
		INNER JOIN ".GD_NAVERCHECKOUT_RETURNINFO." AS R
	...
	$pg->field = "
		...
		R.RequestChannel,
		R.ReturnReason,
		R.ReturnDetailedReason,
		R.HoldbackStatus,
		R.HoldbackReason,
		R.HoldbackDetailedReason,
		R.CollectAddressAddressType,
		R.CollectAddressZipCode,
		R.CollectAddressBaseAddress,
		R.CollectAddressDetailedAddress,
		R.CollectAddressCity,
		R.CollectAddressState,
		R.CollectAddressCountry,
		R.CollectAddressTel1,
		R.CollectAddressTel2,
		R.CollectAddressName,
		R.ReturnReceiveAddressAddressType,
		R.ReturnReceiveAddressZipCode,
		R.ReturnReceiveAddressBaseAddress,
		R.ReturnReceiveAddressDetailedAddress,
		R.ReturnReceiveAddressCity,
		R.ReturnReceiveAddressState,
		R.ReturnReceiveAddressCountry,
		R.ReturnReceiveAddressTel1,
		R.ReturnReceiveAddressTel2,
		R.ReturnReceiveAddressName,
		R.CollectStatus,
		R.CollectDeliveryMethod,
		R.CollectDeliveryCompany,
		R.CollectTrackingNumber,
		R.CollectCompletedDate,
		R.EtcFeeDemandAmount,
		R.EtcFeePayMethod,
		R.EtcFeePayMeans,
		R.RefundStandbyStatus,
		R.RefundStandbyReason,
		R.RefundRequestDate
	";
...
<div class="title title_top">반품리스트 <span>반품 접수된 주문내역을 확인/관리 하는 페이지 입니다.</span></div>
...
<form name="frmNaverCheckout" method="post" target="processLayerForm">
	...
	while ($row = $db->fetch($rs,1)) {
		...
		$disabled = ($row['ClaimStatus'] != 'RETURN_REQUEST' && $row['ClaimStatus'] != 'COLLECTING') ? 'disabled' : '';
	?>
	<tr height="25" bgcolor="#ffffff" bg="#ffffff" align="center">
		...
		<td class="small4"><?=$checkout_message_schema['claimRequestReasonType'][$row['ReturnReason']]?></td>
		<td align="left">
...
<tr><td><img src="../img/icon_list.gif" align="absmiddle"/>반품 접수된 주문내역의 진행 상태를 확인하고 관리합니다.</td></tr>
<tr><td>&nbsp;</td></tr>
<tr><td><img src="../img/icon_list.gif" align="absmiddle"/>반품요청승인 : 반품 요청된 주문건을 접수하고 반품처리로 진행합니다.</td></tr>
</table>
</div>
