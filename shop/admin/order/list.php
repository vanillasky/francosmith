...
<div class="title title_top" style="position:relative;padding-bottom:15px">주문 리스트<span>주문을 확인하고 주문상태를 변경합니다</span>
<a href="javascript:manual('<?=$guideUrl?>board/view.php?id=order&no=2')"><img src="../img/btn_q.gif" border="0" hspace="2" align="absmiddle"/></a>
<div style="position:absolute;left:100%;width:251px;height:44px;margin-left:-260px;margin-top:-15px"><a href="../order/bankmatch.intro.php"><img src="../img/btn_autoaccont.gif"/></a></div>
</div>
...
<tr>
	...
	<td colspan="3" class="noline"><span class="small1" style="color:#5C5C5C;">
	<input type="checkbox" name="chk_inflow[]" value="naver" <?=(in_array('naver',$search['chk_inflow'])?'checked':'')?>><img src="../img/inflow_naver.gif" align="absmiddle"/> 네이버 쇼핑</input>&nbsp;
...
<col width="25"><col width="30"><col width="100"><col width="160"><col><col width="95"><col width="50"><col width="50"><col><col width="55">
...
<?php
	...
	foreach($eachOrderGroup as $eachOrder):
		...
		if($eachOrder['_order_type']=='checkout'):
			...
			$nameOrder = $dormantOrderId = '';
			if($eachOrder['m_id']){
				if($eachOrder['dormant_regDate'] == '0000-00-00 00:00:00'){
					$dormantOrderId = $eachOrder['m_id'];
				}
				else {
					$dormantOrderId = '휴면회원';
				}
			}
			$nameOrder = "<span><span class='small1' style='color:#444444'><strong>".$eachOrder['nameOrder']."</strong>(".$dormantOrderId.")</span></span>";
?>
...
<?php
			...
			$nameOrder = "<span class='small1' style='color:#0074BA'><strong>".$eachOrder['nameOrder']."</strong><br />";
			if($eachOrder['m_id']){
				if($eachOrder['dormant_regDate'] == '0000-00-00 00:00:00'){
					$nameOrder = "<span id='navig' name='navig' m_id='".$eachOrder['m_id']."' m_no='".$eachOrder['m_no']."'>".$nameOrder."(".$eachOrder['m_id']." / ".$r_grp[$eachOrder['level']].")</span></span>";
				}
				else {
					$nameOrder = $nameOrder."(".$eachOrder['m_id']." / 휴면회원)</span>";
				}
			}
			else {
				$nameOrder = $nameOrder."(비회원)</span>";
			}
?>
