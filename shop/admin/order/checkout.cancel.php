...
// 검색절 만듦
	...
	#1. 주문상태
		if ($_GET['ProductOrderStatus'] > -1) {
			...
			$arWhere[] = '('.$checkout_message_schema['extra_productOrderStatusType']['취소전체'].')';
		}
...
// 쿼리 실행
	$db_table = "".GD_NAVERCHECKOUT_ORDERINFO." AS O
		...
		INNER JOIN ".GD_NAVERCHECKOUT_CANCELINFO." AS C
	...
	$pg->field = "
		...
		C.RequestChannel,
		C.CancelReason,
		C.CancelDetailedReason,
		C.CancelCompletedDate,
		C.CancelApprovalDate,
		C.HoldbackStatus,
		C.HoldbackReason,
		C.HoldbackDetailedReason,
		C.EtcFeeDemandAmount,
		C.EtcFeePayMethod,
		C.EtcFeePayMeans,
		C.RefundStandbyStatus,
		C.RefundStandbyReason
	";
...
<div class="title title_top">취소리스트 <span>취소 접수된 주문내역을 확인/관리 하는 페이지 입니다.</span></div>
...
<form name="frmNaverCheckout" method="post" target="processLayerForm">
	...
	while ($row = $db->fetch($rs,1)) {
		...
		$disabled = ($row['ClaimStatus'] != 'CANCEL_REQUEST') ? 'disabled' : '';
	?>
	<tr height="25" bgcolor="#ffffff" bg="#ffffff" align="center">
		...
		<td class="small4"><?=$checkout_message_schema['claimRequestReasonType'][$row['CancelReason']]?></td>
		<td align="left">
...
<tr><td><img src="../img/icon_list.gif" align="absmiddle"/>취소 접수된 주문내역의 진행 상태를 확인하고 관리합니다.</td></tr>
<tr><td>&nbsp;</td></tr>
<tr><td><img src="../img/icon_list.gif" align="absmiddle"/>취소요청승인 : 취소 요청된 주문건을 접수하고 취소처리로 진행합니다.</td></tr>
</table>
</div>
