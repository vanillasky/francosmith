...
// 검색 조건
	...
	$sdate_s = date('Y-m-d', strtotime($_GET['syear'][0].'-'.$_GET['smonth'][0].'-1'));
	$sdate_e = $_GET['syear'][1].$_GET['smonth'][1];
...
function fnDetailStatistic(d) {
	...
	popupLayer('./statistics.order.month.detail.php'+param,750,550);

}
</script>

<div class="title title_top">월별 주문통계 <span>월별 주문통계를 조회/분석 할 수 있습니다.</span> <a href="javascript:manual('<?=$guideUrl?>board/view.php?id=data&no=29')"><img src="../img/btn_q.gif" border=0 align=absmiddle hspace=2></a></div>
...
<form name="frmStatistics" id="frmStatistics" method=get>
	...
	<tr>
		...
		<td colspan="3">
			...
			// 주문 년도
			$_e = time();
			$_s = $db->fetch("SELECT MIN(orddt) as orddt FROM ".GD_ORDER."",1);
			$_s = $_s['orddt'] ? strtotime($_s['orddt']) : $_e;
			?>
			<select name="syear[]">
...
<tr><td><img src="../img/icon_list.gif" align="absmiddle">날짜를 클릭하면 해당 월의 일별 주문통계를 확인 하실 수 있습니다.</td></tr>
<tr><td><img src="../img/icon_list.gif" align="absmiddle">시스템 과부화를 고려하여 검색기간은 최대 1년 단위로 나누어 검색하시고, 엑셀로 파일로 다운로드 하여 활용하시기를 권장 드립니다.</td></tr>
...
<form name="frmExcelQuery" id="frmExcelQuery" method="post" action="indb.excel.statistics.order.month.php" target="ifrmHidden">
