...
function outcallUpload( $_file, $nowPath, $userori = array() ){ // 이미지
	...
	if ( count( $_file ) ){ // Webftp 클래스 정의
		...
		if($cfg['tplSkinMobileWork'] != ""){
			$tplSkin = $cfg['tplSkinMobileWork'];
		}else{
			$tplSkin = $cfg['tplSkinMobile'];
		}

		$webftp = new webftp;
		$webftp->ftp_path = str_replace( $_SERVER['SCRIPT_NAME'], "", $_SERVER['SCRIPT_FILENAME'] ) . $cfg['rootDir'] . '/data/skin_mobileV2/' . $tplSkin; # 스킨경로
