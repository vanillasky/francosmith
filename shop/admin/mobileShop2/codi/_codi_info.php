...
{
	$tmp = dirname(__FILE__) . "/../../../../m2/" . str_replace(".htm", ".php", $_GET['design_file']);

	//if (preg_match("/(board\/|goods\/list\/|goods\/goods_event\.htm|member\/_form\.htm|ord\/|_myBox\.htm|_sub_header\.htm|orderitem\.htm)/i", $_GET['design_file']));
	if (preg_match("/(board\/|goods\/list\/|goods\/goods_event\.htm|ord\/|_myBox\.htm|_sub_header\.htm)/i", $_GET['design_file']));
	else if ( file_exists($tmp) === true ){
		$isPreview = true;
		$real_linkurl = "./../../../../m2/" . str_replace(".htm", ".php", $_GET['design_file'])."?tplSkin=".$cfg['tplSkinMobileWork'];
		...
		$isPreview = true;
		$real_linkurl = "./../../../../m2/html.php?htmid={$_GET['design_file']}&tplSkin=".$cfg['tplSkinMobileWork'];
	}
}
else if ($form_type == 'outSection') {
	@include dirname(__FILE__).'/../../../conf/design_skin_'.$cfg['tplSkinMobileWork'].'.php';
	foreach($design_skin as $key => $val) {
		...
		foreach($val as $key2 => $val2) {
			...
			if ($val2 == $_GET['design_file']) {
				$isPreview = true;
				$real_linkurl = "./../../../../m2/".$val['linkurl']."?tplSkin=".$cfg['tplSkinMobileWork'];
				break 2;
			}
		}
	}
	...
	$isPreview = true;
	$real_linkurl = "./../../../../m2/index.php?tplSkin=".$cfg['tplSkinMobileWork'];
}
?>
<a name="codi_info"></a>
...
<?=gen_design_history_tag('skin_mobileV2', $cfg['tplSkinMobileWork'], $_GET['design_file']); ?>
