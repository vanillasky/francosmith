...
@include_once dirname(__FILE__) . "/../../../conf/design_skinMobileV2_" . $cfg['tplSkinMobileWork'] . ".php";
...
switch ($mode){
	...
	case "create": // 새로운 페이지 추가하기
		...
		$dir = str_replace( $_SERVER['SCRIPT_NAME'], "", $_SERVER['SCRIPT_FILENAME'] ) . $cfg['rootDir'] . '/data/skin_mobileV2/' . $cfg['tplSkinMobileWork'];
		...
		$design_skin[ $_POST['design_file'] ]['linkurl'] = "m2/html.php?htmid={$_POST['design_file']}";
		...
		### 디자인스킨파일 저장
		$qfile->open( dirname(__FILE__) . "/../../../conf/design_skinMobileV2_" . $cfg['tplSkinMobileWork'] . ".php");
		$qfile->write("<?\n" );
	...
	case "popupConf": // 메인팝업창 저장하기
		...
		if($mode == "popupConf"){
			if(!$_GET['design_file']){
				...
				$_POST['linkurl']		= "m2/html.php?htmid=popup/".$_POST['name'].".htm";
			}
			unset($_POST['name']);
		...
		if ( isset( $_POST['content'] ) )
		{
			...
			if ($_GET['gd_preview'] == '1') {
				$dir = str_replace( $_SERVER['SCRIPT_NAME'], "", $_SERVER['SCRIPT_FILENAME'] ) . $cfg['rootDir'] . '/data/_skin_preview/skin_mobileV2/' . $cfg['tplSkinMobileWork'];
			}
			else {
				$dir = str_replace( $_SERVER['SCRIPT_NAME'], "", $_SERVER['SCRIPT_FILENAME'] ) . $cfg['rootDir'] . '/data/skin_mobileV2/' . $cfg['tplSkinMobileWork'];
			}
			...
			if ( $_GET['design_file'] != 'proc/_agreement.txt' )
			{
				...
				if ( $matches[1] ){ // 파일내에 파일정보 있는 경우
					...
					if ( ($_POST['linkurl'] == '' && ereg("popup/",$_GET['design_file'])) || $mode == 'saveas' ) $_POST['linkurl'] = "m2/html.php?htmid={$_GET['design_file']}";
					...
					if ( ($_POST['linkurl'] == '' && ereg("popup/",$_GET['design_file'])) || $mode == 'saveas' )
						$_POST['linkurl'] = "m2/html.php?htmid={$_GET['design_file']}";
					else
		...
		if ( count( $_POST ) > 0 ){
			...
			$qfile->open( $path = dirname(__FILE__) . "/../../../conf/design_skinMobileV2_" . $cfg['tplSkinMobileWork'] . ".php");
			$qfile->write("<?\n" );
			$qfile->write("\$design_skin = array();\n\n" );
		...
		if ($_GET['gd_preview'] !== '1') {
			save_design_history_file('skin_mobileV2', $cfg['tplSkinMobileWork'], $_GET['design_file']);
		}
		// 2013-08-05 slowj 히스토리관리
	...
	case "del": // 페이지 삭제하기
		...
		if ( count( $_BGFILES ) ){ // Webftp 클래스 정의
			...
			$webftp = new webftp;
			$webftp->ftp_path = str_replace( $_SERVER['SCRIPT_NAME'], "", $_SERVER['SCRIPT_FILENAME'] ) . $cfg['rootDir'] . '/data/skin_mobileV2/' . $cfg['tplSkinMobileWork']; # 스킨경로
		}
		...
		### 리턴
		if ( ereg("popup/",$_GET['design_file'] ) ){
			go( '../../mobileShop2/codi.php?design_file=popup/', 'parent' );
		}else{
			go( '../../mobileShop2/codi.php', 'parent' );
		}
		exit;
