...
function file_batch()
{
	...
	fobj.method = "post";
	fobj.action = "../mobileShop2/codi/indb.php?mode=batch";
	fobj.submit();
}
...
function designcodeMove(sobj)
{
	...
	if (path != 'noprint' && path != ''){
		document.location.href='../mobileShop2/iframe.codi.php?design_file=' + path;
	}
...
DCSM = {
	...
	chk: function (fobj)
	{
		...
		document.fm.action = '../mobileShop2/codi/indb.php?mode=saveas&design_file=' + fobj.dir_name.value + fobj.file_name.value + fobj.file_ext.value;
		document.fm.submit();
	...
	file_check: function ()
	{
		...
		var urlStr = "../mobileShop2/codi/_ajax.php?mode=chkFile&dir_name=" + fobj.dir_name.value + "&file_name=" + fobj.file_name.value + "&file_ext=" + fobj.file_ext.value + "&dummy=" + new Date().getTime();
		var ajax = new Ajax.Request( urlStr,
		{
			method: "get",
			onComplete: function ()
...
function file_del(design_file)
{
	...
	fobj.method = "post";
	fobj.action = "../mobileShop2/codi/indb.php?mode=del&design_file=" + design_file;
	fobj.submit();
}
...
DCTM = {
	...
	stylesheet: function ()
	{
		var hrefStr = '../mobileShop2/iframe.css.php';
...
DCRM = {
	...
	{
		var urlStr = "../mobileShop2/codi/_ajax.php?mode=getReplacecode&design_file=" + design_file;
		var ajax = new Ajax.Request( urlStr,
		{
			method: "get",
			onComplete: function ()
