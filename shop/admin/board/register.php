 ...
 * bdEmailNo		이메일 작성
 * bdHomepage		홈페이지 작성
 * bdUseMobile		모바일샵 사용여부
 */

$location = "게시판관리 > 게시판만들기";
include "../_header.php";
...
<div class="title title_top">기본설정<span>커뮤니티 메뉴에서 서비스하는 게시판을 만듭니다 <a href="javascript:manual('<?=$guideUrl?>board/view.php?id=board&no=3');"><img src="../img/btn_q.gif" border="0" hspace="2" align="absmiddle" /></a></div>
...
<tr>
	...
	<tr>
		...
		<td style="padding-left:7"><font class="extext">gallery, photo 스킨 사용시 하단에서  '파일업로드' 기능을 꼭 체크하세요.<br/>※ 모바일샵은 default 스킨과 gallery 스킨만 사용 가능합니다.
</font></td>
	</tr>
	</table>
	</td>
...
<tr>
	<td>모바일샵 사용여부</td>
	<td>
		<label><input type="radio" name="bdUseMobile" value="Y" class="null" <?=$checked['bdUseMobile']['Y']?> /> 사용</label>
		<label><input type="radio" name="bdUseMobile" value="N" class="null" <?=$checked['bdUseMobile']['N']?> /> 미사용</label>
		<font class="extext">모바일샵V2 (default 스킨 제외) 만 사용 가능하며, 모바일샵V1에는 적용되지 않습니다.</font>
	</td>
</tr>
<? if($_GET['mode'] == 'modify'){?>
<tr>
	<td>PC게시판 주소</td>
	<td>(쇼핑몰주소) <?=$cfg['rootDir'].'/board/list.php?id='.$_GET['id']?>&nbsp;<img src="../img/i_copy.gif" align="absmiddle" onclick="prompt('Ctrl+C를 눌러 클립보드로 복사하세요', '<?=$cfg['rootDir'].'/board/list.php?id='.$_GET['id']?>')" style="cursor:pointer" /></td>
</tr>
<tr>
	<td>모바일게시판 주소</td>
	<td>(쇼핑몰주소) <?='/m2/board/list.php?id='.$_GET['id']?>&nbsp;<img src="../img/i_copy.gif" onclick="prompt('Ctrl+C를 눌러 클립보드로 복사하세요', '<?=$cfgMobileShop['mobileShopRootDir'].'/board/list.php?id='.$_GET['id']?>')" style="cursor:pointer" /></td>
</tr>
<?}?>
<tr>
	<td>게시판 위치</td>
...
<tr>
	<td>관리자 표시방법</td>
	<td>
		<input type="radio" name="bdAdminDsp" value="0" class="null" <?=$checked['bdAdminDsp'][0]?> /> 이미지로 표시 <font class="extext">(이미지 등록은 <a href="/shop/admin/board/list.php" target="_new"><font class="small1" color="0074ba">게시판리스트</font></a> 에서 등록가능)</font>
...
<div class="title">권한설정 및 스팸설정<span>커뮤니티 메뉴에서 서비스하는 게시판을 만듭니다 <a href="javascript:manual('<?=$guideUrl?>board/view.php?id=board&no=3');"><img src="../img/btn_q.gif" border="0" hspace="2" align="absmiddle" /></a></div>
...
<tr>
	...
	<td class="noline">
		...
		<table cellpadding="0" cellspacing="0">
		<tr><td style="padding: 5 0 5 3"><font class=extext>이 스팸방지기능은 새로 업그레이드 된 기능입니다. 기능사용 전에 꼭 <a href="javascript:manual('<?=$guideUrl?>board/view.php?id=board&no=9')"><u>패치안내</u></a>를 읽어보세요</font></td></tr>
		</table>
	</td>
</tr>
<tr>
	...
	<td class="noline">
		...
		<div style="padding-top:3"><font class=extext>이 스팸방지기능은 새로 업그레이드 된 기능입니다. 기능사용 전에 꼭 <a href="javascript:manual('<?=$guideUrl?>board/view.php?id=board&no=9')"><font class="extext_l">[패치안내]</font></a> 를 읽어보세요</font></div></td></tr>
		</table>
	</td>
</tr>
<tr>
	<td>부정태그 사용방지</td>
	<td class="noline">
		<input type="radio" name="bdUseXss" value="html" <?=$checked['bdUseXss']['html']?> onclick="document.getElementById('tr_allow_domain').style.display=''" /> 사용함 &nbsp; &nbsp; &nbsp;
		<input type="radio" name="bdUseXss" value="disable" <?=$checked['bdUseXss']['disable']?> onclick="document.getElementById('tr_allow_domain').style.display='none'" /> 사용안함 <font class="extext">

		<table cellpadding="0" cellspacing="0">
			<tr><td style="padding: 5 0 5 3">
				<font class="extext">게시글 등록 시 악성 코드의 삽입을 방지하기 위한 기능입니다</font><br>
			</td></tr>
		</table>
	</td>
</tr>
<tr id="tr_allow_domain">
	<td>허용 태그 설정
	<img src="../img/icons/icon_qmark.gif" style="vertical-align:middle;cursor:pointer;" title="※ 외부 컨텐츠를 쇼핑몰에 연결하기 위해 자주 사용되는 태그에 대한 설정입니다.
- iframe : 게시판 본문에 허용된 도메인(사이트)의 일정 부분을 보여줄 수 있습니다.
- embed : 게시판 본문에 허용된 도메인(사이트)에서 제공하는 동영상, 음악, 플래시 등 미디어 파일을 재생시킬 수 있습니다" />
	<br/>
	...
	<td class="noline">
		...
		<?}?>
		 ...
		 <font class="extext">
		 허용된 도메인의 컨텐츠에 대해 iframe 태그 사용이 가능합니다. 예) youtube.com <br/>
부정한 방법의 태그 사용으로 인하여 혹시 모를 사고를 사전에 방지하기 위하여 허용된 도메인 이외의 컨텐츠는 제한됩니다.</font>
	</td>
</tr>
</table>

<div class="title">리스트화면설정<span>커뮤니티 메뉴에서 서비스하는 게시판을 만듭니다 <a href="javascript:manual('<?=$guideUrl?>board/view.php?id=board&no=3');"><img src="../img/btn_q.gif" border="0" hspace="2" align="absmiddle" /></a></div>
...
<tr>
	<td>리스트<br>
	출력정보 선택<br><font class="small" color="6d6d6d">(list.php)</font></td>
	<td class="noline">
...
<tr id="ListImg">
	...
	<td class="noline">
		...
		&nbsp;<font class="extext">(gallery, photo 스킨만 사용)</font> <a href="javascript:manual('<?=$guideUrl?>board/view.php?id=board&no=3');"><u><font class="small1" color="0074ba"><b>[<u>예제화면보기</u>]</b></a>
	</td>
</tr>
</table>

<div class="title">상세화면설정<span>커뮤니티 메뉴에서 서비스하는 게시판을 만듭니다 <a href="javascript:manual('<?=$guideUrl?>board/view.php?id=board&no=3');"><img src="../img/btn_q.gif" border="0" hspace="2" align="absmiddle" /></a></div>
...
<div class="title">작성화면설정<span>커뮤니티 메뉴에서 서비스하는 게시판을 만듭니다 <a href="javascript:manual('<?=$guideUrl?>board/view.php?id=board&no=3');"><img src="../img/btn_q.gif" border="0" hspace="2" align="absmiddle" /></a></div>
...
<tr>
	...
	<td>
		...
		<select name="bdmaxsize_select" id="bdmaxsize_select" onchange="uploadSizeSelect()" >
			...
			<?php } ?>
		    <option value="o" <?=$selected['bdmaxsize_select']['o']?>>직접설정</option>
		</select>
		<span style="display:none;" id="maxsizeMsg">
			...
			<font class="extext">설정 가능한 최대 용량은 <?=number_format(str_to_byte(ini_get("upload_max_filesize")))?> byte (<?=ini_get("upload_max_filesize")?>) 입니다.</font>
		</span>
	</td>
</tr>
<tr>
...
<div class="title">HTML설정<span>커뮤니티 메뉴에서 서비스하는 게시판을 만듭니다 <a href="javascript:manual('<?=$guideUrl?>board/view.php?id=board&no=3');"><img src="../img/btn_q.gif" border="0" hspace="2" align="absmiddle" /></a></div>
