					...
					for (i = 0; i < chkLen; i++)
					{
						...
						if (response[sno] != null)
						{
							...
							if (gitem['istep'] == '2' && gitem['outOfStock'] == 0) // 품절주문취소요청 버튼 출력
							{
								...
								btn.value = '품절취소요청';
								btn['onclick'] = new Function('e', "popupLayer('../interpark/popup.respCnclOutOfStock.php?ordno=" + gitem['ordno'] + "&sno=" + gitem['sno'] + "&statId=" + statId + "')");
							}
						}
					}
				}
			...
			for ( i = 0; i < ilen; i++ )
			{
				...
				if (l_row.step == 'r')
				{
					...
					if (l_row.clm_tpnm != '출고전주문취소' && i != 0);
						...
						if (l_row.clm_tpnm == '출고전주문취소'){
							btn.style.width = "66px";
							btn.value = '요청승인하기';
							btn['onclick'] = new Function('e', "popupLayer('../interpark/popup.respClaimReqBeforeCancel.php?clmsno=" + i_row.clmsno + "&itmsno=" + i_row.itmsno + "&statId=" + statId + "',0,420)");
						}
						else {
							btn.style.width = "68px";
							btn.value = '승인/거절하기';
							btn['onclick'] = new Function('e', "popupLayer('../interpark/popup.respClaimReqAfterCancel.php?clmsno=" + i_row.clmsno + "&statId=" + statId + "')");
						}
					}
				}
				else if (l_row.step == 'c')
				{
					if (i_row.clm_statnm == '반품/교환수거지시'){
						...
						btn['onclick'] = new Function('e', "popupLayer('../interpark/popup.respClaimStoreComp.php?clmsno=" + i_row.clmsno + "&itmsno=" + i_row.itmsno + "&statId=" + statId + "')");
					}
						...
						btn['onclick'] = new Function('e', "popupLayer('../interpark/popup.respExchangeComp.php?clmsno=" + i_row.clmsno + "&itmsno=" + i_row.itmsno + "&statId=" + statId + "')");
					}
				}
