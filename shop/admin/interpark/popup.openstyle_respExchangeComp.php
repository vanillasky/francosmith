...
function respClaimReq(fObj)
{
	...
	if (confirm("교환(재배송)를 확정하겠습니까?") === false) return false;

	var urlStr = "../interpark/ajaxSock.php?mode=openstyle_exchOutWhCompForComm&clmsno=<?=$_GET['clmsno']?>&itmsno=<?=$_GET['itmsno']?>" + query + "&dummy=" + new Date().getTime();
	var ajax = new Ajax.Request( urlStr,
	{
		...
		onComplete:  function ()
		{
			...
			if ( req.status == 200 )
			{
				...
				if (response == 'succeed'){
					if (statId != '') parent._ID(statId).innerHTML = '<font color=0074BA><b>교환확정전송</b></font>';
					parent.closeLayer();
				}
				else {
...
<div class="title title_top" style="margin-top:10px;">교환(재배송) 확정하기<span>재배송출고지시된 주문을 교환확정합니다.</span></div>
...
<!-- 실행 : start -->
<div style="padding:20 0 5 5"><font class="def1" color="#EA0095"><b><font size="3">②</font> 교환(재배송)여부를 확인하여 송장번호를 입력한 후 [확인]을 클릭합니다.</b></font></div>
<div style="padding:0 0 5 5">
	<div><input type="checkbox" class="null" name="iscomp">교환(재배송)을 확정합니다.</div>
