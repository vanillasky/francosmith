...
if($category) $arr_where[] = $db->_query_print('l.category like [s]', $val_category.'%');
...
if(!empty($category) && is_array($category)) {
	...
	foreach($category as $val_category) {
		if($val_category) {
			$tmp_where[] = $db->_query_print('l.category like [s]', $val_category.'%');
		}
	}
...
$goods_query = $db->_query_print('SELECT DISTINCT g.goodsno, g.goodsnm, g.regdt, g.open, o.price, g.totstock, g.img_s, g.img_l, sg.open_shoptouch, sg.img_shoptouch FROM '.$table.$where.' ORDER BY regdt DESC');
